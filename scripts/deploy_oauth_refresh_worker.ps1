# Deploy the Forge Flow oauth_refresh_worker Cloud Run Job
# + Cloud Scheduler trigger.
#
# Phase 8. Pairs with:
#   * tool/oauth_refresh_worker/Dockerfile — build context.
#   * tool/oauth_refresh_worker/README.md — operator runbook
#     (deploy shape, env names, refresh + auto-disable semantics,
#     refresh-closure-registry follow-up).
#
...
#     script is safe.
#   * Cloud Run JOB (not Service): Cloud Scheduler fires every 10
#     minutes, each invocation claims every near-expiry row then
...
#                               + deploy_first_connect_backfill_worker.ps1)
#   - PGCRYPTO_ENVELOPE_KEY -> forge-flow-staging-pgcrypto-envelope-key
#                              (reused from deploy_staging_proxy.ps1)
#
# Schedule: `*/10 * * * *` UTC (every 10 minutes). The trigger lives
# in northamerica-northeast1 because Cloud Scheduler is not available
# in northamerica-northeast2 today; the Job itself runs in $Region.

param(
  [string] $Project = 'forge-flow-staging',
  [string] $Region = 'northamerica-northeast2',
  [string] $SchedulerLocation = 'northamerica-northeast1',
  [string] $JobName = 'forge-flow-oauth-refresh',
  [string] $SchedulerName = 'forge-flow-oauth-refresh-trigger',
...
# Schedule contract. Locked at this slice: every 10 minutes UTC.
# Update both this constant AND tool/oauth_refresh_worker/README.md
...
try {
  # The 10-minute firing uses `runOnce`, not `daemon`. The image's
  # Dockerfile sets ENTRYPOINT ["/app/oauth_refresh_worker"] and
  # CMD ["daemon"] for local dev; we override with --args to pin
  ...
  # staging proxy + first_connect_backfill_worker use; reusing it
  # keeps the firewall a single line.
  & $gcloud run jobs $jobVerb $JobName `
    --project $Project `
    --region $Region `
    --service-account $ServiceAccount `
    --image $Image `
    --command /app/oauth_refresh_worker `
    --args runOnce `
...
Write-Host ' - Refresh closure registry wiring follow-up — see tool/oauth_refresh_worker/README.md'
