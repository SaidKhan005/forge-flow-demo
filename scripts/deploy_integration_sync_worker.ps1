# Deploy the Forge Flow integration_sync_worker Cloud Run Job
# + Cloud Scheduler trigger.
#
# Phase 8 Wave B `8.spine-bridge.0` recurring poll worker. Pairs with:
#   * tool/integration_sync_worker/Dockerfile — build context.
#   * tool/integration_sync_worker/README.md — operator runbook.
#
# Posture (mirrors scripts/deploy_first_connect_backfill_worker.ps1):
...
#     script is safe.
#   * Cloud Run JOB (not Service): Cloud Scheduler fires every 5 min,
#     each invocation drains every connected connection's poll then
#     exits.  See README.md "Deploy shape" for the rationale.
#
# Required env names (loaded from the secrets file, NEVER printed):
#   - POSTGRES_URL
#   - PGCRYPTO_ENVELOPE_KEY  (Phase 8 — required since the binder
#                              builder constructs `VendorCredentialBroker`
#                              against this key. Mirrors the staging
#                              proxy required list.)
#
...
# disabled vendors that the worker logs as poll_error with a clean
# reason instead of stack-traced StateErrors):
...
#
# Schedule: `*/5 * * * *` UTC (every 5 minutes). The trigger lives in
...
param(
  ...
  [string] $JobName = 'forge-flow-integration-sync',
  [string] $SchedulerName = 'forge-flow-integration-sync-trigger',
...
# Schedule contract. Locked at 8.spine-bridge.0: every 5 minutes UTC.
# Update both this constant AND tool/integration_sync_worker/README.md
...
# Optional vendor app credential bundles. Each entry is wired into
# Cloud Run --set-secrets ONLY when the matching env var is present in
# the local secrets file. Absent bundles surface as warn-disabled
# vendors at runtime, and the worker logs poll_error rows for those
# vendors with a clean reason instead of stack-traced StateErrors.
# Mirrors the staging proxy + first-connect backfill deploy script
# pattern; the secret names are the same so a single rotation can sync
# all three deploys.
...
try {
  # Production pins to `runOnce`. The image's Dockerfile sets ENTRYPOINT
  # ["/app/integration_sync_worker"] and CMD ["daemon"] for local dev;
  # we override with --args to pin production to the bounded one-tick
  # mode (see README.md).
  #
  ...
  # anchor admits this Job too.
  & $gcloud run jobs $jobVerb $JobName `
    --project $Project `
    --region $Region `
    --service-account $ServiceAccount `
    --image $Image `
    --command /app/integration_sync_worker `
    --args runOnce `
...
Write-Host ' - Adapter factory wiring: backed by the binder builder (single source of truth shared with the proxy + first-connect backfill worker).'
