...
- `dart run tool/migration_drift_scanner.dart --fix --strict-docs`: partial. It updated `scripts/postgres_staging_setup.ps1` to the new cutoff, then failed strict-docs because shared authority/backlog docs still reference `202605191000_per_daypart_v1_r7f_data_accuracy_precedence_fix.sql`. Those tracker/backlog docs were intentionally not edited per orchestrator instruction.
- `dart run tool/migration_cutoff_lint.dart`: pass.
