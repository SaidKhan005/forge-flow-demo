...
| `adp` | mTLS | `oauth` | NOT wired (intentional per ADP partner-ops out-of-band cert rotation) | adapter wins on authMode label, runtime is split | Update `docs/integrations/adp/oauth_shape.md` to spell out the split: ADP IS oauth+mTLS, refresh closure is intentionally absent because cert rotation is partner-ops out-of-band, F&F surfaces near-expiry as connection error. Add a `kVendorsWithoutRefreshClosure` reason field. |
| `opentable` | "internal" | `oauth` | NOT wired (intentional per `OpenTableTransport.refresh` doing it inline) | runtime is internal-to-adapter rather than external broker | Update `docs/integrations/opentable/oauth_shape.md` to call out that broker has no closure for vendor `opentable`; `OpenTableTransport.refresh` owns rotation. |
| `sevenrooms` | "transport" | `oauthOrKeyPaste` | NOT wired (per `oauth_shape.md` transport-layer cron at hour:05) | broker has no closure; transport cron handles it | Update `docs/integrations/sevenrooms/oauth_shape.md` if transport-layer cron is not yet running (verify); otherwise add a registry entry that maps to the transport surface. |
...
| `provider_credentials_repository.dart` | 202 | yes (`test/provider_credentials_repository_test.dart`) | P3C atomic-rotation probe + advisory-lock probe | P0 | Phase 3C ran the per-tenant Future-lock + atomic-rotation snapshot probe against the worker's `buildProductionRefreshClosures` via in-memory fakes. Real-DB tests need to cover (a) the `oauth_refresh_advisory_lock` migration semantics under N parallel refreshes, (b) ciphertext atomicity between the simulated vendor RTT and commit, (c) the `consecutive_refresh_failures` counter contract. Extend the existing test against staging. |
...
2. `provider_credentials_repository.dart` — extend existing test with the per-tenant Future-lock + atomic-rotation contracts Phase 3C exercises against staging Postgres + the `oauth_refresh_advisory_lock` migration.
