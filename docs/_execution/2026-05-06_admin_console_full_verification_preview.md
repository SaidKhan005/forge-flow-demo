...
- Latest verified `origin/master` after final fetch: `290ba1d11cb02392e52736dbf44a8350b03602eb`
- Previous code-fix commit used for first evidence: `5571a0a4` (`Tolerate live auth admin preview payloads`)
- Current verification source before the final closeout commit: `4c1add64994dd59e2db349585bcc11fca3383b38` plus the Members/auth-scope fixes recorded in this PR.
- Final branch commits over master:
  ...
  - final closeout commit in this PR - Members invite payloads, cross-operator auth scope, local filters, and execution evidence
...
- Postfix stack redeploy command rerun from this worktree after the Members/auth-scope fixes. The admin deploy succeeded, but the wrapper failed its final runtime check after an automatically promoted proxy revision returned `cors_origin_not_allowed` for the preview admin origin.
- Direct proxy redeploy command used for diagnosis:
  `powershell -ExecutionPolicy Bypass -File scripts\deploy_staging_proxy.ps1 -Project forge-flow-staging -Region northamerica-northeast2 -Service forge-flow-preview-backend-surface-additions-proxy -ServiceAccount forge-flow-staging-admin@forge-flow-staging.iam.gserviceaccount.com -SecretPrefix forge-flow-staging- -ProxyBaseUriEnvVarName FORGE_FLOW_PREVIEW_BACKEND_SURFACE_ADDITIONS_PROXY_BASE_URI -ProxyEnvironment preview-backend-surface-additions -AdminCorsAllowedOrigins "https://forge-flow-preview-backend-surface-additions-admi-rf7nosnoka-pd.a.run.app" -VpcConnector ff-staging-proxy-egress -VpcEgress all-traffic -MinInstances 0 -SkipApiEnable -SkipSecretManagerSync`
...
- Final admin revision: `forge-flow-preview-backend-surface-additions-admin-00014-kw2`, traffic 100 percent
...
- Database mode: staging secrets and shared staging data. The first pass was read-only; the operator later gave explicit mutation approval in-thread for this verification pass.
...
- Mutation controls were exercised only on safe staging/test targets after explicit approval. Destructive controls such as feature-flag high-impact toggles and member soft-delete were inspected or stopped at confirmation instead of submitted.
...
- Postfix/mutation-pass evidence directory: `.codex_appdata/admin-console-full-verification/browser-postfix-20260506T1428`.
- Browser route/action JSON: `.codex_appdata/admin-console-full-verification/browser-postfix-20260506T1428/postfix-results.json`.
...
Postfix Browser Use evidence lives under `.codex_appdata/admin-console-full-verification/browser-postfix-20260506T1428`.
...
- Members no longer crashes on live payload shape. After the final admin rebuild, pending invites render `Operator Manager` instead of a raw role UUID. One scoped test context still produced a browser `Failed to fetch` while Cloud Run logged a 200 response; this is tracked as live proxy/CORS revision drift because verified traffic revision `00021-tmz` passes admin-auth preflight and the final admin route reloads cleanly when the context is selected against that revision.
...
- Members invite creation now sends routed auth fields (`role_id`, `scope_type`, `location_id`) while preserving readable `role_key` and `primary_location_id`.
- Members lists now carry selected operator/location as routing context without treating that location as the visible Location filter unless the user chooses a filter.
- Members rows and invites now prefer `role_key`/`role_label` before falling back to raw `role_id`, avoiding UUID role labels in the UI.
- Admin auth operations now honor explicit admin target `operator_id` and `location_id`/`primary_location_id` for cross-operator admin actions while preserving `ff_support` read-only and requiring `super_admin` for cross-operator writes.
- The live invite mutation for `codex.invite.restoredcors.test.20260506@example.com` succeeded and rendered in Pending invites with a human-readable `Operator Manager` role after the admin rebuild.
- `scripts/deploy_preview_stack.ps1` now checks `/v1/admin/auth/users` preflight in addition to `/v1/admin/operators`, so future preview deploys catch the admin-auth CORS regression before reporting success.
...
- Mutations performed after explicit approval were limited to staging/test/QA-safe actions: QA operator create/edit/suspend/reactivate, QA location edit, Data Accuracy override note, Polling assignment, and one test invite. An accidental Barrio suspend during the button sweep was immediately reactivated and is captured in the Browser evidence.
- High-impact feature flag toggles, provider rotations, soft delete, corpus commits, role hierarchy writes, session revokes, and support reset actions were treated as approval-stop or backend-gated surfaces during Browser Use.
...
Passed:
- `flutter analyze`
- `flutter test test\admin` - 357 tests
...
- `flutter build web --release --target=lib\main_admin.dart --dart-define=ADMIN_DEMO_AUTH=false --dart-define=ADMIN_PROXY_BASE_URI=https://forge-flow-preview-backend-surface-additions-prox-rf7nosnoka-pd.a.run.app`
- Postfix targeted route pack - 175 tests:
  `flutter test test\proxy\admin_cors_routes_test.dart test\proxy_auth_operations_route_test.dart test\proxy_auth_operations_route_grants_test.dart test\services\proxy_feature_flags_routes_test.dart test\services\proxy_corpus_admin_routes_test.dart test\proxy_integration_admin_routes_test.dart test\proxy\data_accuracy_admin_routes_test.dart test\admin\services\members_admin_gateway_test.dart test\admin\screens\members_admin_screen_test.dart`
...
Postfix performance:
- Command:
  `dart run tool\perf_gate\staging_console_probe.dart --run --enforce-budgets --admin-url=https://forge-flow-preview-backend-surface-additions-admi-rf7nosnoka-pd.a.run.app --proxy-url=https://forge-flow-preview-backend-surface-additions-prox-rf7nosnoka-pd.a.run.app --admin-revision=forge-flow-preview-backend-surface-additions-admin-00014-kw2 --proxy-revision=forge-flow-preview-backend-surface-additions-proxy-00021-tmz --label=admin-console-full-verification-postfix --write-json=build\perf_gate\admin_console_full_verification_preview_postfix.json`
- JSON evidence: `build\perf_gate\admin_console_full_verification_preview_postfix.json`
- Result: budgets passed
- Probe summary:
  - `admin_index_c1`: p50 62.9 ms, p95 288.0 ms, errors 0.0 percent
  - `admin_index_c4`: p50 64.8 ms, p95 187.7 ms, errors 0.0 percent
  - `admin_mainjs_gzip_c4`: p50 854.1 ms, p95 1255.6 ms, p99 1493.8 ms, errors 0.0 percent, bytes 1173276
  - `proxy_readyz_c1`: p50 64.9 ms, p95 225.1 ms, errors 0.0 percent
  - `proxy_readyz_c4`: p50 64.3 ms, p95 196.3 ms, p99 1192.8 ms, errors 0.0 percent
