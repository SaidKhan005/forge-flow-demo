# Oracle MICROS Simphony — Live Verification Checklist

**Vendor ID**: `oracle_micros_simphony`
**Engineering slice**: `8.OR`
**`*.live.sandbox` slice**: `8.OR.live.sandbox` — pending
**`*.live.prod` slice**: `8.OR.live.prod` — pending
**Lifecycle**: `documented` (will promote on first verification slice)

This checklist is filled in as the `8.OR.live.sandbox` and
`8.OR.live.prod` slices run. The engineering slice ships this file
...
- [ ] **Polling resume.** Worker restart resumes from persisted
      cursor (no rewind, no skip). Poll-only Simphony makes this
      load-bearing.
      Test: `<test_path>:<line>`
...
- [ ] **Idempotency.** Same `chkNum` + `lastUpdatedUTC` twice → single
      canonical write (UNIQUE).
      Test: `<test_path>:<line>`
- [ ] **Sanity hook.** Future-dated guest check from sandbox →
      sanity_log row + drop.
      Test: `<test_path>:<line>`
- [ ] **Field-mapping diff.** Observed sandbox response matches every
      `documented_per_oracle_micros_simphony_v2` constant in fixtures.
...
- [ ] **Operator dashboard chrome.** `MetricCardNotYetAvailable`
      ...
      degradation line.
      Walkthrough: `docs/_walkthroughs/8.OR.live.sandbox.md`
...
## Production verification (`8.OR.live.prod`)
...
- [ ] **Auth round-trip.** Production client_credentials → access
...
- [ ] **Idempotency.** Same vendor event ID twice → single canonical
      write.
      Test: `<test_path>:<line>`
- [ ] **Sanity hook.** Future-dated guest check from production →
      sanity_log row + drop.
      Test: `<test_path>:<line>`
- [ ] **Field-mapping diff.** Observed production response matches
      every `documented_per_oracle_micros_simphony_v2` constant in
...
- [ ] **Operator dashboard chrome.** `MetricCardNotYetAvailable`
      flips to live number; top-left pill drops the corresponding
      degradation line.
      Walkthrough: `docs/_walkthroughs/8.OR.live.prod.md`
