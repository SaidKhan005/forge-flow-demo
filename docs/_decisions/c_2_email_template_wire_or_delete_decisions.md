...
| E | `vendor_webhook_signature_alert` | **DELETED — landed via C-2-Del.** Operator picked DELETE (rely on Cloud Logging alerts + signature-verifier unit tests). Template `.md` + `EmailTemplateIds.vendorWebhookSignatureAlert` + subject map entry + pressure inventory row removed. ~Mechanical delete; mirrors A2.2 PR #540 shape. |
...
| G | `tos_version_updated_notice` | **DELETED — landed via C-2-Del.** Operator picked DELETE (rely on the in-app accept-screen gate at `lib/operator_web/screens/tos_accept_screen.dart` as the sole TOS-update operator signal; the gate fires the next time the operator signs in after a new TOS version lands). Template `.md` + `EmailTemplateIds.tosVersionUpdatedNotice` + subject map entry + pressure inventory row removed. ~Mechanical delete; mirrors A2.2 PR #540 shape. If a publish workflow ships later, the template can be re-added alongside it. |
...
| **E** | `vendor_webhook_signature_alert` | **DELETED (this PR)** | `C-2-Del` landed: template + renderer constant + subject + inventory row removed. Cloud Logging alerts cover signature failures. |
...
| **G** | `tos_version_updated_notice` | **DELETED (this PR)** | `C-2-Del` landed: template + renderer constant + subject + inventory row removed. In-app accept-screen gate is the sole TOS-update operator signal. |
