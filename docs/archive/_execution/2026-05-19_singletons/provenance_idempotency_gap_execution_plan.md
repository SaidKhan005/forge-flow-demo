...
- Manual covers PATCH now requires `Idempotency-Key`.
- Manual covers PATCH now uses the admin idempotency ledger for reserve,
  replay, and conflict behavior.
- Same key plus same body replays the first response and does not run the
  gateway write again.
- Same key plus different body returns `409 idempotency_key_conflict`.
- `effective_data_accuracy_settings_v` now appends server source metadata for
  covers source, wage source, and walk-in handling mode.
- Proxy data-accuracy reads now come from the effective view directly, so
  final values and source metadata travel together.
- Shared Dart models and mobile sync snapshots parse provenance fields without
  breaking older responses.
- Operator Web and Admin show source labels only when the server sends
  metadata.
- Migration queue docs and setup cutoff now include
  `202605190900_per_daypart_v1_r7e_data_accuracy_provenance.sql`.

## New Follow-Up Found

- The broader `data_accuracy_settings` and
  `data_accuracy_service_period_settings` PATCH routes still receive
  client idempotency keys but do not yet ledger/replay them at the route layer.
- Their database writes are value-idempotent today, but they do not provide
  cached response replay or same-key/different-body conflict behavior.
- This slice fixed the tracked manual-covers gap only; the broader route
  hardening should be a follow-up because it may require a client/back-compat
  decision for older callers that do not send `Idempotency-Key`.

## Verification Run

- `flutter test test/per_daypart_v1_r7e_data_accuracy_provenance_test.dart test/per_daypart_v1_r7b_proxy_covers_keyed_test.dart test/services/sync/http_sync_proxy_client_test.dart test/operator_web/widgets/covers_source_toggle_test.dart test/operator_web/widgets/wage_source_toggle_test.dart test/operator_web/widgets/walk_in_handling_card_test.dart test/admin/data_accuracy_ux_framework_polish_test.dart test/proxy/mobile_operational_sync_routes_test.dart`
- `dart run tool/migration_drift_scanner.dart --fix --strict-docs`
- `dart run tool/migration_cutoff_lint.dart`
- `dart run tool/postgres_import_lint.dart`
- `dart run tool/permission_key_lint.dart`
