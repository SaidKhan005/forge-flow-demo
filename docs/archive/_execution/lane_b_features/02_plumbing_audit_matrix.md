...
| Lens 13 Audit | `tool/advisor_proxy/proxy_bootstrap.dart:3640` | Closure carries the drift observation: operator-location admin audit fan-out emits `actorKind: 'user'` → maps to `actor_kind = 'team_member'` instead of `'forge_admin'`. Carried in `docs/archive/_audits/post_codex_wave_2026-05-13/pr_490_audit.md`. | Lane B owns the targeted fix because audit-actor honesty intersects B8 audit filter. Slice B1.b — one-line flip with audit-rebake guarded against chain integrity. Lift onto `docs/POST_HARDENING_FOLLOWUPS.md` if not picked up. |
