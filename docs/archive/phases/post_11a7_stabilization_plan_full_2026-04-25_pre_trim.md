...
4. **Per-operator data isolation is non-negotiable, and the schema is
   ...
   `location_id`. Corpus / methodology stays `operator_id`-scoped.
   Phase 9 turns enforcement on. 11b cannot ship to multiple operators
   until Phase 9 RLS is real and tested. Decided 2026-04-25 — retrofit
   is not an option once facts exist.
...
6. **Advisor speaks in recommendations, not commands; F&F never acts on
   the operator's behalf in the launch product.** Posture decided
   2026-04-25: option B (recommendation with operator approval). The
   advisor surfaces methodology-grounded recommendations with provenance
   and confidence framing ("I recommend X because Y"); the operator
   evaluates and decides whether to act. F&F does not take action on
   the operator's behalf. Workflow automation (Phase 12 post-launch)
   extends this with explicit per-action operator consent — same
   posture, just covering automated actions. **Liability:** F&F
   provides advisory information; operator decisions and outcomes are
   the operator's sole responsibility. T&Cs codify this in Phase 9.8
   compliance scope (flagged for that phase). E&O insurance evaluation
   also lives in 9.8.

7. **F&F holds all provider keys server-side. No BYO-key path.**
   Decided 2026-04-25: restaurant operators do not manage API keys.
   F&F absorbs LLM cost as COGS via subscription pricing. Proxy
   backend in `11a.10` (Cloud Run service) brokers every LLM and
   embedding call; F&F's master Anthropic + Voyage keys live there.
   Client app never holds production keys — dev `--dart-define` is
   local development only. Production keys live in Cloud Run env /
   KMS. This makes `11a.10` a launch-blocking foundation, not a
   "later if demand justifies" item. Cost containment (Haiku-default
   tiered routing, prompt caching, per-operator monthly cost cap)
   ships with the proxy.

8. **AI infrastructure is general-purpose, not advisor-specific.**
   Decided 2026-04-25: today's advisor product (11b) is one consumer
   of the AI plumbing; tomorrow's workflow automation (Phase 12
   post-launch) is another. Provider abstractions are named for
   capability, not product surface — `LLMProvider`,
   `EmbeddingProvider`, `RerankProvider`, `DataSourceProvider`. The
   `AdvisorAnswerProvider` rename to `LLMProvider` lands inside
   `7.57.3c` or a small `7.57.3d`. Workflow automation builds on the
   same proxy + provider abstractions + MCP tool layer; no new
   infrastructure required for the Phase 12 product surface.
...
**Scope:**
- Update `CLAUDE.md` with five rules:
  1. **Service layer split:** `lib/data/` is legacy and frozen (only
     ...
     folder).
  2. **RLS-ready schema:** operator-scoped Postgres fact tables
     include `(operator_id, location_id)` plus an RLS policy stub
     from day one. Single-location operators run with a default
     `location_id`. Corpus / methodology stays `operator_id`-scoped.
  3. **Time-storage rule:** operator-scoped Postgres fact tables
     store source-truth instants as `TIMESTAMPTZ` (UTC) plus a
     denormalized `business_date` `DATE` column computed at write
     using `location.timezone` (IANA, per-location) +
     `business_day_rollover_hour`. `TIMESTAMP WITHOUT TIME ZONE` is
     banned in operator-scoped tables. `business_date` is
     write-once, never recomputed at read.
  4. **Repository pattern (two-layer defense):** app code
     reads/writes operator-scoped Postgres tables only through
     `OperatorScopedRepository<T>` (or equivalent), which injects
     `(operator_id, location_id)` from the current `OperatorContext`.
     Raw `package:postgres` imports forbidden outside
     `lib/infrastructure/persistence/postgres/` — CI lint enforces.
     Postgres RLS is the safety net under the repository, not the
     primary defense.
  5. **Phase doc hygiene:** slices < 1 week of work AND < 5 files of
...
**Acceptance:**
- Five rules + eight hard promises (above) present in `CLAUDE.md` and
  `PROJECT_TRACKER.md`.
...
| Production AI/vendor secret gateway | `11b` runtime contract before advisor calls; Phase 8 for vendor connectors | Server-side gateway (Supabase Edge Function / Cloudflare Worker / backend API); Flutter release builds never receive Anthropic/Voyage/vendor keys via `--dart-define` |
...
27 distinct concerns surfaced across the planning rounds; every one
has a slot.
...
| Flag | F7 | Production AI/vendor secret gateway | `11b` / Phase 8 |
