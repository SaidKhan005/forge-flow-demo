...
`tool/advisor_proxy/advisor_proxy.dart` lines 353-462. The live apply of
`202604280000` through `202604280013` is complete on staging and Production1
after explicit approval. Remaining live-closeout work is human-gated Cloud
Armor enforcement monitoring and iOS physical device-matrix execution. New gaps
surfaced by the parallel-merge audit are tracked as B33 (usage_logs two-slot
mirror) and B34 (audit_logs attribution contract clarification) below.
...
- Older B6/B8/B14/B15/B17/B19/B20/B21 backlog text has been reconciled with
  ...
  result docs below. Production1 approval was granted in-session and
  `202604280000` through `202604280013` are now applied and verified on staging
  and Production1. Final local analyzer/test gates are green after the staging
  repairs and tracker updates.
- Live MFA adapter gap narrowed on 2026-04-28: the proxy now has an Identity
  ...
  staging user.
- Production1 migration apply completed in the 2026-04-29 session after
  explicit approval. Staging and Production1 both applied and verified
  `202604280000` through `202604280013`; `ltree` was allow-listed in Azure
  `azure.extensions`, and rollup cron jobs were registered from the `postgres`
  maintenance database with `cron.schedule_in_database`.
...
active; enforcement flip deferred to backlog/monitoring.
...
- 2026-04-28 read-only preview-log sample covered
  `2026-04-28T19:30:57Z` through `2026-04-29T00:14:25Z`: 187 sampled load
  balancer entries, 18 preview WAF matches, 0 non-probe `/readyz` preview
  matches, and 0 `/v1/auth/*` or `/v1/admin/auth/*` preview matches. This is
  promising but below the required 3-day evidence window, so enforcement stays
  backlog-only.
...
- Backlog: review Cloud Armor preview logs after at least 3 days of normal
  traffic, then decide whether to enforce.
...
Status: completed in the 2026-04-29 session after explicit Production1
live-mutation sign-off.
...
- `docs/archive/phases/phase_9/phase_9_production1_migration_apply_result.md`

Resolution:

- Staging and Production1 applied `202604280000` through `202604280013`.
- Azure `ltree` allow-listing and Azure `pg_cron` maintenance-database
  scheduling are documented in the runbook and result file.
- Post-apply object/extension/cron checks passed on both environments.

Gate: closed. Future Production1 mutations still require a fresh live-mutation
...
Status: queued as a post-apply regression hardening sweep.

Needed before: launch/perf hardening, not before the already-completed
Production1 apply.
...
| `B35` Production1 apply runbook/apply | complete | (future live mutations need fresh approval) |
| `B36` cross-tenant RLS isolation test sweep | queued | (post-apply regression hardening) |
...
- In-app `auth-smoke@forgeflow.dev` login/logout through the deployed proxy
  has passed (see `docs/archive/phases/phase_9/phase_9_in_app_auth_smoke_result.md`).
...
- Next live-closeout work is Cloud Armor enforcement backlog/monitoring and the
  iOS physical device matrix. Production1 `202604280000` through
  `202604280013` apply is complete.
- Cloud Armor preview-log review/enforcement, route-level reCAPTCHA token
  ...
  Production1 deploy remain gated follow-ups.
