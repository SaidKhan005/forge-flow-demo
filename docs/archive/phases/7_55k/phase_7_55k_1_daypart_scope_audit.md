# Phase 7.55k.1 â€” Daypart Scope Audit
...
Variance, History, and Learn â€” what scope each surface actually
...
| Primary Driver | WTD aggregate | `WeekData.primaryLeverId` resolved via `LeverCards` | Single lever id from the WTD aggregate driver detection. Whole-day scope â€” not daypart-aware. |
| Full Week Projection label | Section label | Static text | Says "FULL WEEK PROJECTION" â€” implies a forecast, which is structurally correct. |

### Variance â€” Full Week Projection
...
| Closed daypart rows | Daypart detail â€” closed truth | `ShiftRecord` with `status == 'closed'` | Actual covers, sales, hours, lever. Target fields from the locked weekly plan snapshot via the shift's `targetProfileId`. |
| Open daypart rows | Daypart detail â€” live partial | `OpenShiftSnapshot` converted via `CurrentWeekState.shiftRecordFromSnapshot` | May carry live partial actuals. `primaryLever` is hardcoded `'ON_MODEL'` â€” a placeholder, not a driver claim. See `7.55m.4` audit. |
| Projected daypart rows | Daypart detail â€” plan placeholder | `OpenShiftSnapshot` with `status == 'projected'` converted the same way | No actuals. Forecast covers and scheduled hours from the plan. `primaryLever` is `'ON_MODEL'` â€” inert placeholder. |
...
| Teaching summary â€” leak | Historical pattern | Most common unfavorable lever + top 2 daypart labels | Daypart labels come from `HistoryPatternRecord.fullLabel` (e.g. "Saturday Dinner"). |
| Teaching summary â€” benchmark | Historical pattern | Most common favorable lever + top 2 daypart labels | Same source. |

**Overstatement gaps:**
- "Benchmark dayparts" are frequency labels, not evidence-backed
  summaries. Saying "Saturday Dinner is a benchmark daypart" means
  it appeared favorably the most times â€” it does not carry average
...
### Variance â€” History Data Pipeline
...
**What is missing:**
- No metric context in `HistoryPatternRecord` â€” only lever + benchmark flag.
...
| Surface | Scope | Authority | Notes |
|---------|-------|-----------|-------|
| Header | Week count | `weekCount` from `WeekRecord` list length | Correct â€” just a count. |
| Benchmark Set card | Canonical benchmark truth | `LearnBenchmarkContext` from `LearnBenchmarkContextService.resolve()` | Canonical path: active profile â†’ active/recovered cycle â†’ persisted `BenchmarkSelectionSummary`. Bridge values only for explicit bridge-only mode or genuine no-profile bootstrap â€” not a normal resolution tier. Source label, targets, range quality all from persisted authority. This is the strongest-sourced surface in the Learn tab. (See `phase_7_55l_8_learn_bridge_closeout.md`.) |
...
| Component | Scope | Input | Output | Notes |
|-----------|-------|-------|--------|-------|
| `LearnBenchmarkContextService.resolve()` | Canonical benchmark | Active profile â†’ active/recovered cycle â†’ persisted summary | `LearnBenchmarkContext` | Canonical path is profile â†’ cycle â†’ summary. Bridge is not a normal third tier â€” it fires only for explicit bridge-only mode or genuine no-profile bootstrap. Recovery paths (8c1, 8d) repair missing cycle/summary state. |
...
- "Repeatable Wins" implies proven repeatability. The current evidence
  is lever-frequency counts from `HistoryPatternRecord` â€” it shows
...
### 7.55k.2 â€” Service-Period Decoupling Plan
...
### 7.55k.3 â€” Daypart Pattern Summary Model
...
The audit confirms this is the root model gap â€” every downstream
overstatement traces back to pattern records not carrying metric
context.

### 7.55k.4 â€” Variance Full Week Projection Semantics
...
### 7.55k.5 â€” History Benchmark Dayparts Upgrade
...
### 7.55k.6 â€” Learn Repeatable Wins Upgrade
...
### 7.55k.7 â€” Interim Visibility Rules
...
- `docs/archive/phases/7_55k/phase_7_55k_1_daypart_scope_audit.md` (this doc)
