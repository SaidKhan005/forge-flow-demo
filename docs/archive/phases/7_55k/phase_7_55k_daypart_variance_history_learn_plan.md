...
Status: complete (`7.55k.1` through `7.55k.8a` complete)
...
Variance now has an explicit Full Week read seam, but it still has mixed scope by design:

- This Week / WTD is aggregate week-to-date.
- Full Week Projection renders day rows and expandable daypart rows through
...
- Mixed day rows now surface compact non-final composition.
- Open/projected rows no longer surface `ON_MODEL` as row-scope driver truth.

This is now correct enough structurally for Variance. The remaining `7.55k`
work is no longer row-provenance honesty inside Full Week; it is downstream
History / Learn evidence hardening.
...
`7.55m` is now complete. The runtime-truth cleanup that needed to land before
downstream semantics work has been delivered:

- shared date / business-date authority (`7.55m.1` / `7.55m.1a`)
- mock replay drift contract (`7.55m.2` / `7.55m.2a`)
- truthful Shift clock / time behavior (`7.55m.3`)
- driver parity audit (`7.55m.4` / `7.55m.4a`)
- Benchmark OPZ truth audit (`7.55m.5` / `7.55m.5a`)
- Plan / Benchmark / Settings surface cleanup (`7.55m.6` / `7.55m.6a`)

Full closeout summary: `docs/archive/phases/7_55m/phase_7_55m_7_runtime_truth_closeout_handoff.md`

`7.55k` now begins from cleaned runtime-truth boundaries.

A separate future runtime-foundation lane now exists for the timing and
service-period seams that should not be improvised inside `7.55k`:
...
- `ShiftRecord` = closed or projected daypart-shaped record with persisted `business_date` (since 7.55f)
...
Use it for closed facts, open snapshots, reservation snapshots, schedule daypart rows, and read-model joins. `ShiftRecord.businessDate` is already persisted (landed in 7.55f). The remaining identity work is in derived/history layers (`HistoryPatternRecord`) and read-model joins that still key on `weekId|dayLabel|daypart`.
...
Delivered: `docs/archive/phases/7_55k/phase_7_55k_1_daypart_scope_audit.md`
- Full surface classification for Variance (This Week, Full Week, History), Learn
- Overstatement gap table with 7 identified gaps and their owners
- Concrete handoff map linking each gap to 7.55k.2â€“7.55k.7
...
Delivered: `docs/archive/phases/7_55k/phase_7_55k_2_service_period_decoupling_plan.md`
- Full identity seam inventory (which models carry `businessDate`, which do not)
- Full hardcoded definition/order seam inventory (6 label helpers, 3 order maps, `WeekDayOrder.daypartsFor`, `morning` gap analysis)
- Proposed `ServicePeriodKey` and `ServicePeriodDefinition` model shapes
- Read-service boundary before Phase 10.5
- Phase ownership across 7.55f, 7.55i, 7.55k, 7.55l, 7.55m, 10.5
- Concrete handoff map for 7.55k.3 through 7.55k.8
...
Delivered: `docs/archive/phases/7_55k/phase_7_55k_3_daypart_pattern_summary_model.md`
- `DaypartPatternSummary` model: aggregate recurring-bucket summary with counts, averages, dominant levers, exemplar IDs
- `DaypartPatternSummaryBuilder`: closed-only input, grouped by `restaurantId + dayLabel + daypart`, deterministic ordering and tie-breaking, sample-threshold parameter
- `7.55k.3a` evidence-contract cleanup:
  - unknown lever IDs excluded from benchmark/leak evidence
  - deterministic exemplar fallback IDs no longer depend on grouped input order
- 37 focused tests covering filtering, grouping, averages, lever detection, ordering, threshold, fixture-seed scenario, unknown-lever exclusion, and exemplar stability
- Current History / Learn consumers remain on `HistoryPatternRecord` (migration in 7.55k.5 / 7.55k.6)
...
Status: implemented in `7.55k.4` with final honesty cleanup in `7.55k.4a`.

This slice should introduce a dedicated `VarianceWeekProjectionReadService`
and a small explicit read model for Full Week row semantics. The mixed
closed/open/projected/day-total meaning should not remain embedded in widget
helpers alone.
...
Implementation should route the screen through the read service / read model
instead of leaving source-truth decisions in widget helpers.

Acceptance:

- `VarianceWeekProjectionReadService` (and, if needed, a narrow row read model)
  owns row provenance and day-total reconciliation
...
- placeholder `ON_MODEL` row-driver behavior does not masquerade as row-scope truth
...
Status: implemented in `7.55k.5` with final evidence-honesty cleanup in `7.55k.5a`.
...
```text
BENCHMARK DAYPARTS
Sat Dinner - 5/8 wins, avg 4.8 CPLH, 188 SPLH
Thu Dinner - 4/6 wins, avg 4.6 CPLH, 181 SPLH
```
...
Status: implemented in `7.55k.6` with final teaching-scope honesty cleanup in `7.55k.6a`.
...
Status: implemented in `7.55k.7` with final visibility-honesty cleanup in `7.55k.7a`.
...
Status: implemented in `7.55k.8` with final integration-doc truth cleanup in `7.55k.8a`.
...
7.55f delivered persisted `business_date` on `ShiftRecord`. 7.55k builds on that landed seam. The remaining work is in derived/history layers and read-model joins, not in closed-shift persistence itself.
