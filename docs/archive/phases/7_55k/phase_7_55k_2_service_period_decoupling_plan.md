# Phase 7.55k.2 â€” Service-Period Decoupling Plan
...
| `ReservationBookSnapshot` | `restaurantId + businessDate + daypart` | Already the closest to the target identity. No `weekId` or `dayLabel` â€” keyed directly on business date + daypart. |
...
- `HistoryPatternRecord` is the primary model gap â€” it drops
...
### `WeekDayOrder.daypartsFor(...)` â€” The Central Fixture

Location: `lib/data/legacy_fixture_data.dart:1507â€“1523`
...
The same `lunch â†’ 'Lunch'` / `dinner â†’ 'Dinner'` / `late_night â†’
...
Maps `lunch â†’ 'L'`, `dinner â†’ 'D'`, `late_night â†’ 'LN'`. Default
returns first letter uppercase â€” `morning` would get `'M'`, which is
...
Location: `lib/screens/schedule_builder.dart:255â€“259`
...
would fall back to weight `1.0` â€” functional but not operationally
tuned.
...
|----------|------------------|
| `MockIntegrationReplaySeed.weekSlots` | 14 slots per week: Monâ€“Thu lunch+dinner, Fri lunch+dinner+late_night, Sat dinner+late_night, Sun dinner. No morning. |
...
1. `WeekDayOrder.daypartsFor(...)` â€” does not return `morning` for any
   day
2. `_DaypartChips._abbr()` â€” `morning` â†’ `'M'`, ambiguous with Mon
3. All 3 daypart order maps â€” `morning` would sort last (index 99)
4. `MockIntegrationReplaySeed.weekSlots` â€” no morning slots generated
5. All fixture `_daypartRatios` â€” no morning ratios
...
removed â€” they just stop being the durable identity for daypart facts.
...
class ServicePeriodDefinition {
  ...
  final String? localStartRule; // e.g. '06:00' â€” nullable until Phase 10.5
  final String? localEndRule;   // e.g. '11:00' â€” nullable until Phase 10.5
...
`localStartRule` and `localEndRule` are nullable now â€” they become
...
### `7.55k.3` â€” Daypart Pattern Summary Model (next)
...
aggregates across those facts â€” its identity is the recurring bucket,
...
### `7.55k.8` â€” Integration Implications
...
- `docs/archive/phases/7_55k/phase_7_55k_2_service_period_decoupling_plan.md` (this doc)

## Cross-References

- `docs/archive/phases/7_55k/phase_7_55k_1_daypart_scope_audit.md` â€” surface classification
  and overstatement gaps
- `docs/archive/phases/7_55k/phase_7_55k_daypart_variance_history_learn_plan.md` â€” parent
  plan
- `docs/archive/phases/7_55i/phase_7_55i_pre_7_55i3_integration_daypart_checkpoint.md` â€”
  app-owned service-period guidance
- `docs/contracts/phase_7_55_target_cycle_weekly_plan_rules.md` â€” locked
