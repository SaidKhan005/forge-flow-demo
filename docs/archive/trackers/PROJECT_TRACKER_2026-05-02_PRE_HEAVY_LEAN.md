...
Updated: 2026-05-02 (post-batch lean: 11A.3b/4b/4c/7/UX.health, G.2, 7.58.5 accepted; HARD-A through HARD-H hardening sprint complete)
Owner: You
...
- `docs/POST_HARDENING_FOLLOWUPS.md` - audit-surfaced items not in
  any HARD-* contract (Production1 migration apply gap, RLS gaps on
  proxy_requests / feature_flags, 11A.3a operator-picker, three
  admin gateways missing Idempotency-Key, MFA + repository test
  coverage holes). Drive priority from here when scoping next slices.
...
- **Current phase:** `9` framework + `9.0Σ.b-k` accepted; `9.UX.*` family
  shipped on operator surface; `11A.0-6` foundation active. Recent batch
  (2026-05-01 → 2026-05-02): `11A.3b` corpus admin, `11A.4b/4c`
  ...
  row purity. **Hardening sprint complete (HARD-A → HARD-H, PRs #41–49).**
  All 8 closed contracts in `docs/contracts/hardening_*.md`. Next: 7.58
  Primary Driver audit + remaining 11A operations console slices.
...
- **`cutover.0a` + `cutover.0a.pg` complete 2026-05-01** per
  ...
  with lowercase action group ID. **21 migrations remain pending Production1
  apply** (`202604280014` through `202605021000`, including the 9.0Σ.h2
  audit-privacy role, 11A.1/3a/3b/4/4b/4c/7 columns, B41/B42/B43, and the
  HARD-B + HARD-H schemas). Apply event must batch in lex order. Inventory
  MD: `~/.forge_flow/secrets/inventory/secrets_inventory.md`.
...
| `11A.0-2`, `11A.3b`, `11A.4`/`4b`/`4c`, `11A.7`, `11A.UX.health` | accepted on master | `phase_11A_operations_console_plan.md` |
| `11A.3a` (corpus version ledger) | partial — corpus upload/diff/rollback shipped; graph commit blocked on operator-picker screen (single-screen follow-up) | `phase_11A_operations_console_plan.md` |
| `11A.5` debug console, `11A.6` cost dashboard | not started — placeholder routes only; `11A.6` also blocked on B44/B45/B47 producers | `phase_11A_operations_console_plan.md` |
| `11A.8`/`9`/`10` | not started — no scaffolding yet (post-launch polish) | `phase_11A_operations_console_plan.md` |
