...
| Gap 31 | `shift_close_authority` operator-editability | **Delete the setting entirely. Auto-derive from per-vendor capability + operator's business-day-start time.** Add a `CloseAuthorityCapability` enum (sidecar pattern matching `lib/services/integration/labor_wage_source_class.dart`) keyed by vendor adapter — F&F integration testing classifies each vendor as reliable-finalization vs unreliable. Aggregator + close logic reads the enum: vendor reliable → use vendor finalization timestamp; vendor unreliable → close at the operator's business-day-start time. No operator UI, no F&F admin UI, no carve-out needed. **Business-day-start becomes the single anchor for everything that closes daily** (matches WTD reset + weekly plan lock + cycle rollover gating). | Slice 1.5 absorbs: drop `shift_close_authority` column from `restaurant_timing_configs`; remove admin write path at `operator_location_admin_screen.dart:3719`; add `CloseAuthorityCapability` sidecar; update aggregator + close logic to read it. |
...
| Gap 35 | Operator-web Benchmarks override write-seam | **Cut the operator-web override entirely.** Mobile Baseline Manager star-shift selector is the only override path. Operator-web Benchmarks screen becomes read-only (still shows current targets, but no pin-to-override button). | Slice 2 extension: remove override write path from `benchmarks_screen.dart` + `operator_web_benchmarks_gateway.dart`; keep `benchmark_overrides` as historical read-only compatibility; remove active repository write helpers so new adjustments flow through selected-star target cycles instead of whole-day pooled rows. |
...
|---|---|---|---|---|
| 19 | **No production-wired `CanonicalFactPeriodResolver` instance threaded into the proxy binder.** The typedef + the binder's optional resolver parameters exist (`lib/services/integration/projecting_canonical_sink.dart:50-58`); the production call site at `tool/advisor_proxy/main.dart:1625-1629` does not pass any of the three resolvers, so `projectorWiringActive` evaluates `false` in production. No production-wired instance exists. (Refinement per architecture verification audit 2026-05-15.) | `tool/advisor_proxy/main.dart:1625-1629`; `tool/advisor_proxy/phase_8_vendor_integration_factories.dart:327-346`; `lib/services/integration/projecting_canonical_sink.dart:50-58` | **Production cutover blocker.** Not in scope for per-daypart V1, but flagged as a precondition for any real vendor → closed-shift flow at launch. | Out of scope — separate production cutover work stream. |
...
| 24 | **`closeShift` hardcodes `14 shifts/week` (2 periods × 7 days)** as the gate for upserting `WeekRecord`. With 4-period schedules or weekend brunch the gate never fires. Related debt comment at `lib/services/shift_service.dart:507` (`'14 shifts' close-detection one layer above is unowned debt`) — Slice 1.5 should scrub both sites. | `lib/services/shift_service.dart:294-300`; `lib/services/shift_service.dart:507` | Same bug class as Gap 15 (`['lunch','dinner','late_night']` hardcode). | **Slice 1.5** (bundled with bucketing fix) |
...
| 35 | **Closed by removing active whole-day override writes.** Operator-web no longer writes pooled `target_cplh/splh/ppa` scalars to `benchmark_overrides`; old HTTP write routes return HTTP 410, and the repository is read-only over historical rows. | Historical refs: `lib/operator_web/screens/benchmarks_screen.dart:128-184`; `tool/advisor_proxy/operator_benchmark_overrides_routes.dart`; `lib/infrastructure/persistence/postgres/repositories/benchmark_overrides_repository.dart` | Mobile Baseline Manager selected-star writes remain the active override path and write per-period target cycles. | **Closed/superseded** |
...
| 43 | **`wage_at_lock_time_json` writer is `WeeklyPlanSnapshotService`** (confirmed by architecture verification audit 2026-05-15). | `lib/services/weekly_plan_snapshot_service.dart` | Naming locked. | **Slice 1** (write per-(day, period) demand-derived values + `wage_at_lock_time_json`) |
...
## Vendor sink business_date gaps (post-Slice-1.5 end-to-end verification, 2026-05-15)

Surfaced by the write-side surface-coverage audit run after Slice 1.5 merged. The audit found that all vendor sinks write `business_date` using a legacy hour-only precision path while the operator's canonical configuration carries `HH:MM` precision plus hierarchy inheritance. None of the per-daypart V1 slice work depends on these gaps — for an operator with an on-the-hour cutoff and a single location, end-to-end attribution is correct. Sub-hour cutoffs, multi-location inheritance, and Tock reservations expose drift.

| # | Gap | Source | Severity | Resolved in |
|---|---|---|---|---|
| 45 | **`tock_reservation_postgres_sink.dart` uses raw UTC calendar date for `business_date`.** No timezone conversion, no rollover. Any Tock reservation in any timezone other than UTC gets wrong attribution. Acknowledged in the file's own comment at lines 156–160 as a stub pending `8R.TC.live.sandbox`. | `lib/infrastructure/persistence/postgres/tock_reservation_postgres_sink.dart:148-162` | Vendor-broken (Tock not generally available; soft-blocked). | **CLOSED 2026-05-15 — Slice 7a merged at `baa4047a` (PR #766).** Sink now mirrors OpenTable / Libro pattern: injects `IanaTimezoneConverter`, reads `(timezone, business_day_rollover_hour)` from `public.locations` inside the tenant transaction, projects via `toBusinessDate`. Default fallback `('UTC', 4)` matches Libro. Audit doc: `docs/_audits/per_daypart_v1/slice_7a_tock_business_date.md`. |
| 46 | **All 19 vendor sinks use `locations.business_day_rollover_hour` (INTEGER 0..23) instead of `business_timing_profiles.business_day_start_local_time` (TIME, HH:MM).** Sub-hour cutoffs (e.g., 04:30) silently truncate to integer hour; operator→org_unit→location inheritance is bypassed (sinks only read the locations row). Server-side SQL trigger `phase_8_set_business_date_hardening.sql:126-147` mirrors the same drift. (Note: actual sink count is **20** verified via `git ls-files`; the "19" in the original gap row is close — the Aloha/NCR/Voyix family ships three reseller sink variants.) | All 20 sinks under `lib/infrastructure/persistence/postgres/*_postgres_sink.dart`; `lib/services/integration/iana_timezone_converter.dart:86-109`; `db/migrations/202605071900_phase_8_set_business_date_hardening.sql:126-147` | Architectural drift — affects any operator with sub-hour cutoff or multi-location inheritance. | **Slice 7b LOCKED 2026-05-15 — option (b).** Operator decision: per-sink resolve via `BusinessTimingProfilesRepository.listCandidateProfilesForLocation` → `BusinessTimingProfileResolver.resolve` → `BusinessDateResolver.resolve`. Sub-decisions: **(b1)** SQL trigger stays as legacy backup (no PL/pgSQL helper rewrite); **fallback hour converges on `4`** across all sinks + the SQL trigger (matches Libro + operator-default `business_day_start_local_time = '04:00'`); **`locations.business_day_rollover_hour` deprecated, NOT dropped in 7b** (drop deferred to a follow-up after a deprecation cycle). **Sequencing: dispatches AFTER Main's Slice 1 (PR #767) merges** to avoid concurrent sink-write churn. Research doc: `docs/_audits/per_daypart_v1/slice_7b_research_2026_05_15.md`. PR locking decision: see commit history. |
| 47 | **`locations.business_day_rollover_hour` vs `business_timing_profiles.business_day_start_local_time` drift, no sync.** Populated by different code paths (location admin vs business-timing UI), no trigger keeps them consistent. | `lib/infrastructure/persistence/postgres/repositories/locations_repository.dart`; `lib/infrastructure/persistence/postgres/repositories/business_timing_profiles_repository.dart`; `db/migrations/202605060000_phase_business_timing_live_schema.sql:537-548` | Latent consistency risk. | **Folds into Slice 7b option (b) — LOCKED 2026-05-15.** Sinks read the canonical profile chain; the legacy column is deprecated. Drift becomes moot because no live reader consults the legacy column after Slice 7b. |

## End-to-end verification 2026-05-15 (post-Slice-1.5 merge)

Two parallel surface-coverage audits run against `master` at `d392d4d1` (Slice 1.5 merged). Verifies the operator scenario "shift 23:00 Tuesday → 03:30 Wednesday with business-day-start 04:00 and Late Night daypart 22:00–02:00 (rolls past midnight)" end-to-end.

**Read side — verdict: clean.** All seven primary surfaces (Shift whole-day card, Shift daypart cards, Variance WTD, Variance Full Week Projection, Plan day-by-day, History week detail, Data Alignment audit panel) correctly chain to the operator's business-hours configuration. No surface re-computes `business_date` independently. Surface B (Shift daypart cards) intentionally re-buckets canonical POS/labor facts via `DaypartBucketer` at read time using operator's IANA + `businessDayStartLocalTime` from `RestaurantTimingConfig`; refuses to fall back to device clock when IANA is missing. Surface E (Plan) and G (audit) key their daypart sub-row splits off operator-configured periods via the shared `DaypartPlanAllocator` seam. The 23:00 Tue → 03:30 Wed scenario validates: both edges resolve to Tuesday business date, Tuesday Late Night absorbs 23:00–02:00 (with `rollsPastMidnight=true` constructing the `[Tue 23:00, Wed 02:00)` calendar interval per `DaypartBucketer._periodIntervalsOnBusinessDate:343-345`), the 02:00–03:30 sliver becomes a `non_service` gap segment excluded from CPLH/SPLH denominators per the Jim Taylor rule.

**Write side — verdict: gaps 45/46/47 surfaced.** Every vendor sink writes `business_date` via the legacy hour-precision path. For an operator with on-the-hour cutoff + single location + non-Tock vendors, 18 of 19 sinks produce the correct answer (Tock is broken even in this best case). Sub-hour cutoffs, multi-location inheritance, and any Tock reservation expose drift.

**Deferred regression test.** A focused test locking the 23:00 Tue → 03:30 Wed scenario was deferred from PR #763 per operator decision "merge then fix." Test scope when authored: extend `test/services/integration/canonical_fact_to_closed_shift_input_test.dart` with a new group covering (a) POS check at 23:00 Tue → Tuesday Late Night, (b) labor punch 23:00 Tue → 03:30 Wed → 3 hours Tuesday Late Night + 1.5 hours non-service, (c) reservation at 01:30 Wed → Tuesday Late Night.

**CLOSED 2026-05-15 — PR #768 merged at `6e843798`.** Group P (4 sub-tests P.1–P.4) added to `test/services/integration/canonical_fact_to_closed_shift_input_test.dart` (since split into focused `canonical_fact_*_test.dart` files) covering all three locked behaviors plus a composite end-to-end assertion. P.2 directly asserts the Jim Taylor non_service exclusion (`actualFohHours == 3` not 4; `actualFohLaborDollars == $66` not $99; `LaborPunchSegment(servicePeriodId: null, minutes: 90)` for the 02:00 → 03:30 Wed sliver). 33/33 tests pass (29 pre-existing + 4 new). Audit doc: `docs/_audits/per_daypart_v1/regression_test_slice_1_5_late_night_crossing.md`.
