...
terms at business, org-unit, or location scope with lower-scope override
precedence, RLS, and operator-leading indexes; then replaces all-time target
uniqueness with non-overlapping effective-date windows so future contracts can
be scheduled without overwriting the current active contract; schema + RLS +
proxy-writing surface, gated on operator approval), preceded by
...
preceded by `202605240900_b10_1_vendor_applicability_location_scope.sql`
