# Phase 7.55k.4 — Variance Full Week Projection Semantics
...
- `closed` — locked closed truth from finalized shift facts
- `open` — live in-progress snapshot context, not final truth
- `projected` — planned / forecast placeholder context, not actual
  performance
...
projection — it mixes closed truth with non-final context.
...
- Projected rows no longer say "Projected from 60-day baseline" — that
...
- New copy: "Projected from weekly plan. Actuals populate when shift
  closes." — matches the real runtime authority.
- Open rows: "Live shift in progress. Finalizes on close." — unchanged.
...
- The open-row expanded detail previously used `CURRENT` as its
  ...
  (forecast covers, theoretical labor %) — not live actual metrics.
- New header: `PLAN CONTEXT` — honestly describes the column content.
- Projected rows still use `PROJECTED` as their header — unchanged.
...
- `docs/phases/7_55k/phase_7_55k_4_variance_full_week_projection_semantics.md` (this doc)
...
- `docs/phases/7_55k/phase_7_55k_1_daypart_scope_audit.md` — overstatement gaps
- `docs/phases/7_55k/phase_7_55k_2_service_period_decoupling_plan.md` — identity shapes
- `docs/phases/7_55k/phase_7_55k_3_daypart_pattern_summary_model.md` — pattern summary
