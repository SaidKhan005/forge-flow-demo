# Phase 7.55k.3 — Daypart Pattern Summary Model
...
itself aggregates across those facts — its identity is
`restaurantId + dayLabel + daypart`.

### Relationship to existing models

- `HistoryPatternRecord` — lightweight per-shift signal (lever + boolean).
  Remains in place; not removed or migrated in this slice.
- `HistoryPatternBuilder` — existing builder. Remains in place as the
  active path for `HistoryTeachingAnalyzer` and `LearnTeachingAnalyzer`.
- `DaypartPatternSummary` — new aggregate carrying counts, averages,
  dominant levers, and exemplar source shift IDs.
- `DaypartPatternSummaryBuilder` — new builder producing summaries from
...
Output is sorted by canonical day order (Mon–Sun via `CanonicalDayOrder`)
...
- `7.55k.5` — History benchmark dayparts upgrade: will consume
  `DaypartPatternSummary` for evidence-backed benchmark daypart labels.
- `7.55k.6` — Learn repeatable wins upgrade: will consume
  `DaypartPatternSummary` for metric-grounded coaching copy.
- `7.55k.7` — Interim visibility rules: will use `closedShiftCount`
  against the sample threshold for gating and early-signal labels.

## Files

- `docs/phases/7_55k/phase_7_55k_3_daypart_pattern_summary_model.md` (this doc)
