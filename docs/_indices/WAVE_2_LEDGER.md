...
| EN-3-FU | Lane Q | Main | operator | parked | – | EN-3 | EN-3 worker disclosure 2026-05-14: full `NotificationEventFanout` production binding (~6-8 file slice). EN-3 (#729) shipped telemetry warnings on the two production main paths (`first_connect_backfill_worker/main.dart`, `audit_anchor/main.dart`) that emit `notif.event.unwired` structured logs when the fanout would have fired. Full production wiring requires building 4 Postgres-backed seams: user directory, preference reader, push outbox, email outbox. Defer to V1.1. |
