...
| A11.1 | Slice A11.1 | Claude | Small | Medium | operator | A0 merged | audit-pending | #522 | Production `proxy.session_record.incomplete{route, missing_field}` gauge — observability-only at `POST /v1/auth/session/login`; awaiting operator approval (proxy-touching) |
