...
| A5+A8 | Slice A5+A8 | Codex | Medium | Medium | operator | A0 merged | audit-pending | #527 | Schema versioning post_deploy convention + `audit_logs` UPDATE allowlist lint + `--require-expand-contract` scanner mode; grandfathered to B11.1; awaiting operator approval (schema/audit-chain guardrail) |
...
| A9.1 | Slice A9.1 | Codex | Medium | Medium | operator | A0 merged | audit-pending | #530 | `lib/data` rehome — 3 frozen-legacy files moved (app_defaults + cross_axis_pair_catalog → `lib/domain/constants/`; mock_integration_replay_seed → `lib/dev/`). 72 files / +90/-90 (mostly import-path updates). 3 active contract docs updated. `lib/data/` empty post-merge. Awaiting operator approval. |
