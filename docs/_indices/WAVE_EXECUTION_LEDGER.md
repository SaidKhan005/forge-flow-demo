...
| A2.2 | Slice A2.2 | Codex | Medium | Medium | operator | A2.1 merged | audit-pending | #540 | Email pipeline wire-or-delete — deleted 2 dormant SendGrid templates (`password_reset_request.md`, `operator_admin_invite.md`) + their registry entries; preserved `operator_invite_first_admin.md` as admin SendGrid test fixture. Live flows use Firebase action-link email (verified). 21/21 tests pass. Awaiting operator approval (email is operator-visible). |
