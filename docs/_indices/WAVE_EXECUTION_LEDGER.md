...
| A4.2 | Slice A4.2 | Claude | Medium | Medium | operator | A4.1 merged | audit-pending | #552 | Performance fixes landing all 3 A4.1 ranked recs: R1 (Postgres pool default 4→20), R2 (4 shift_dashboard 30s timers → 1 shared `_ShiftDashboardTicker` ValueNotifier), R3 (`healthProducerConcurrency` env-resolved). 11 files / +608/-288. Behavior-preserving (prod env already pins pool=20; ticker cadence identical; /health is manual). All test runs clean (3 new + 8/9/15/60 existing); pre-existing baseline failures snapshot-verified NOT introduced. Audit doc: `docs/_audits/post_codex_wave/pr_552_a4_2_performance_fixes_audit.md` |
...
| B11.2 | B11.2 | Claude | Medium | High | operator | B11.1 merged | audit-pending | #550 | RFC 9470 step-up challenge **scaffold** — schema (`auth_step_up_challenges` table + wrapper-only RLS + operator-leading indexes) + pure-function policy + dispatch router + RFC 9470 header builder + 27 tests pass. Auth posture unchanged on master until B11.2.b wiring lands. Three deferrals (per-route wiring, Postgres gateway binding, client adapter) disclosed but need durable logging. Pattern B partial (narrative only, no audit tables — 1/3 violation counter). Audit doc: `docs/_audits/post_codex_wave/pr_550_b11_2_step_up_challenge_audit.md` |
