...
- Forecast covers remain app-derived from POS history unless a later phase explicitly changes that product decision.
- Vendor forecast fields, if available, should be documented as optional context until the app intentionally adopts them.
- Reservation data remains contextual unless a later phase explicitly changes forecast demand policy.
...
- Baseline demand: closed covers and sales over the rolling 60-day window.
- Schedule: forecast covers from POS history, forecast sales derived by app from covers * target PPA.
...
| Schedule | historical covers/sales for demand history | schedule hours if comparing published staffing | optional future book context | forecast covers, forecast sales, SchedulePlan |
...
| Variance WTD | closed actuals and finalization | closed actual hours/labor dollars | optional explanatory context | lever math, locked target comparison |
