...
| Daypart-assignable timestamps (7.55k) | opened/closed timestamps for service-period bucketing | Backfill | POS | Cannot assign daypart-level pattern depth | **Degrade** — whole-day patterns work; daypart depth unavailable until 7.55k |
...
| Daypart-assignable hours (7.55k) | hours mapped to service periods by time range | Backfill | Labor | Cannot assign daypart-level labor patterns | **Degrade** — whole-day patterns work; daypart labor depth unavailable until 7.55k |

### Required Reservation Capabilities

None required. Optional explanatory context for future History depth (7.55k).
...
| Capability | Description | Depends On |
|---|---|---|
| Benchmark context | Currently from BaselineData (6 reads); target: from TargetCycle | TargetCycle (7.55l.1) / ActiveTargetProfile |
| Recurring leak identification | From History pattern records across multiple weeks | Closed pattern history |
| Repeatable wins identification | Benchmark dayparts that recur across weeks | Closed pattern history |
| Coaching summaries | Combined teaching analysis | Pattern records + benchmark context |
| Evidence-backed coaching (7.55k) | Source ids, daypart counts, and exemplar tracing | Closed POS + Labor with source ids |
...
## Cross-Cutting: Daypart Capability Check (7.55k Dependency)

Phase 7.55k requires evidence-backed daypart depth for History and Learn.
This adds a specific capability check on top of the basic feature requirements.

| Capability | Required From | Purpose | Fallback |
|---|---|---|---|
| Timestamp-based daypart assignment | POS: opened/closed timestamps | App maps sales/covers into service periods | If no timestamps: daypart depth unavailable; whole-day only |
| Daypart-level labor mapping | Labor: hours by time range and role | App maps labor hours into service periods | If no time-range granularity: daypart labor unavailable |
| Post-close correction feed | POS + Labor | Ensures benchmark history does not go stale | If no correction feed: accept initial close values |
| Source ids for exemplar tracing | POS + Labor | Dedup and trace exemplar daypart records | If no source ids: daypart records cannot be traced to originals |

Architecture decision confirmed by
`phase_7_55i_pre_7_55i3_integration_daypart_checkpoint.md`:
...
## Cross-Cutting: TargetCycle and WeeklyPlanSnapshot Dependency

Multiple surfaces depend on TargetCycle and WeeklyPlanSnapshot, which are
not yet implemented (owning phase: `7.55l`).

| Planned Model | Surfaces That Will Consume It | Current Interim Source |
|---|---|---|
| `TargetCycle` | Benchmark, Schedule, Variance WTD, Variance History, History, Learn | `ActiveTargetProfile` (reads from BaselineData bridge) |
| `WeeklyPlanSnapshot` | Schedule, Shift, Variance WTD, Variance History, History | `SchedulePlan` (resolved live, not locked) |

Integration readiness note: even with live POS/Labor transport, the
TargetCycle and WeeklyPlanSnapshot runtime architecture must be in place
before the app can properly lock and compare targets and plans. This is
a `7.55l` prerequisite, not a Phase 8 task.
