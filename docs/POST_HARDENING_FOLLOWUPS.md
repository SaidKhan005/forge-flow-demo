...
| `202605161800_gap_b2_wage_role_rows_hierarchy_scope.sql` | GAP B2 HP #11 wage slice. ADDITIVE `ALTER TABLE public.wage_role_rows` adding `scope_type` (default `'location'`), `org_unit_id`, `inherited_from_scope_id` + scope-payload CHECK + `org_units` composite FK + operator_id-leading scope index, mirroring `202605131550_benchmark_overrides_hierarchy.sql`. Relaxes the `wage_role_rows_per_tenant` RLS policy from operator+location to operator-only (benchmark_overrides posture; location authority still proxy-enforced). Backward compatible: legacy rows + proxy writes that omit the columns stay Location-scoped via the column default. The other 4 HP #11 tables remain the broader B1 follow-up. | code-ready |
