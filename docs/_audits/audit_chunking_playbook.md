...
- **Counting failing test FILES instead of CASES.** B2.3 worker (PR #603) disclosed "5 pre-existing test failures in `test/proxy/`" — investigation in PR #610 found the real picture was **2 stale snapshots across 3 cases in 2 files** (3 of the listed "5 files" had no actual failures). Same shape as Bundle 33's "5 failures in admin_cors_bootstrap_test" inflation (actually 1 test case with 5 stacked `expect` calls). When disclosing failures: count CASES, list files only as locator metadata.
- **Slice verification command lists that omit tests hitting touched seams.** A3.3 (PR #572) narrowed the catch site at `tool/advisor_proxy/advisor_proxy.dart:5605` from `catch (_)` to `on Exception catch (_)`, but A3.3/A3.4 verification command lists did NOT include `test/proxy/registry_proxy_health_check_store_test.dart` even though it exercises that exact catch site with `throw StateError(...)` test fakes. The failures didn't surface until PR #603's full-sweep run a day later. **For each touched seam, list every test in the repo that hits it.** Use `Grep` on the touched symbol/line to enumerate, not just the slice's own tests.
