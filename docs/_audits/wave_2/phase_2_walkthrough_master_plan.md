...
| **Mobile (Pixel_9 emulator-5554)** | Main Claude (this lane) | ✅ **WALKTHROUGH DONE; output is Per-Daypart Targets V1 plan.** Mobile walkthrough Passes 1–2 surfaced four UX concerns + the dominant architectural finding: the recommendation engine already computes per-service-period targets cleanly but pools them into whole-day scalars before persistence (`recommended_benchmark_selection_service.dart:305-332`), violating Promise 3. Every downstream surface (Benchmark Daypart Breakdown, Plan day-by-day sub-rows, Shift daypart cards, Variance Full Week Projection) loses per-period resolution. Three parallel surface-coverage audit agents (integration plumbing / settings / read-write consumers) consolidated 44 gaps. **Walkthrough output:** `docs/phases/per_daypart_targets_v1/per_daypart_targets_v1_plan.md` — 14 architectural decisions locked; 9-slice sequence (0 → 1 → 1.5 → 2 → 2.5 → 3 → 4 → 5 → 6); reusable surface-coverage audit method appended; 4 operator decisions queued before slice dispatch. Five mobile UX bug-fixes already merged this session (PRs #757–#760 + #756 salvage). | Master tip `69de2201` (Variance Daypart toggle cut) + PRs #758–#760 | **Lane closed for walkthrough.** Mobile work transitions to Per-Daypart Targets V1 implementation under `docs/phases/per_daypart_targets_v1/`. Pass 3 (permission edges + Demo→Live observation) re-runs after V1 slices land. |
...
- 2026-05-15: Claude2 closed 2 parked V1.1 admin follow-ups on branch `claude2/admin-v1-1-followups`: `RP-15-FU-admin-demo-override-fixture` (admin Data Accuracy now seeds 1 historical override on Demo Diner Co. → Toronto Yorkville so the audit panel renders "1 event / Latest: Applied data accuracy override" + the per-location row shows the overridden lunch covers / wage values; new test `data_accuracy_admin_gateway_initial_audit_log_test.dart` pins the contract) + `Support-FU-admin-walkthrough` (per-location `_LocationActionButton` now sets `excludeFromSemantics: true` on its outer `Tooltip`, so every action button's visible Text label — "Support view", "People", "Access", etc. — becomes the sole semantic leaf for walkthrough automation + screen readers). Both changes pass `flutter analyze` + the relevant admin test suites. Live walkthrough verification under Claude Preview MCP was blocked by a separate headless-Chromium issue (the page reports `document.hidden=true`, pausing Flutter's render pipeline indefinitely; in-page visibility overrides post-bootstrap don't recover the lifecycle state); code-anchored via the gateway-level + screen-level test suites. `MO-5b-FU-admin-my-account` from Claude2's round-2 list was already swept by Main's PR #747 (commit `80382b62` touched `lib/admin/screens/my_account_admin_screen.dart` — "Two-factor sign-in is required for every Forge & Flow admin." is now the canonical body copy); the V1.1 admin backlog is now empty.
...
| `RP-15-FU-admin-demo-override-fixture` (filed admin lane round 2, 2026-05-14 evening; deferred V1.1) | Admin Covers and Wage Data Accuracy (RP-15 admin-undo path) cannot be exercised live in the current demo fixture — Demo Diner Co. → Toronto Yorkville has no existing override seeded, so the audit history is empty ("0 events / No admin overrides recorded yet.") and the undo state-machine cannot be driven | `_defaultDataAccuracyDemoGateway` seed block in `lib/admin/admin_routes.dart` | Walkthrough fidelity (admin parallel of operator-web `RP-15-FU-cap-state-manager-session`) | **FIX LIVE** (admin V1.1 round) | ✅ **RESOLVED** via PR #751 (2026-05-15). `InMemoryDataAccuracyAdminGateway` now accepts an `initialAuditLog` constructor param; the demo gateway seeds one historical `admin.data_accuracy.override` event on Demo Diner Co. → Toronto Yorkville (lunch covers `vendor → manual`, wage `vendor → manual_mix`, F&F Support actor, dated 2026-05-10 14:17 UTC) plus the matching `initialSettings` row so the per-location screen renders the overridden values + a "1 event / Latest: Applied data accuracy override" audit panel. New test `test/admin/data_accuracy_admin_gateway_initial_audit_log_test.dart` pins the seed→reader→subsequent-write contract. |
| `Support-FU-admin-walkthrough` (filed admin lane round 2, 2026-05-14 evening; deferred V1.1) | The per-location "Support view" button at `operator_location_admin_screen.dart:3326-3334` could not be reached by headless browser automation — its outer `Tooltip` exposed the tooltip message as the parent semantic label, masking the button's "Support view" Text label. | `_LocationActionButton` in `lib/admin/screens/operator_location_admin_screen.dart` | Walkthrough fidelity (admin-side semantic-tree discoverability) | **FIX LIVE** (admin V1.1 round) | ✅ **RESOLVED** via PR #751 (2026-05-15). `_LocationActionButton`'s `Tooltip` now sets `excludeFromSemantics: true`; the inner `OutlinedButton.icon` Text label ("Support view", "People", "Access", "Edit", "Make primary", "Remove", "Timing", "Data accuracy", "Vendor connections", "Polling + pricing", ...) is now the sole semantic leaf for every per-location action button. Walkthrough automation + screen readers find buttons by their visible label without the tooltip drift. Existing `operator_location_admin_gateway_test.dart` 29 tests still pass. |
