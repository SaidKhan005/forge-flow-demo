...
##### Still to drive in a future session (require additional dev patches)
- [ ] **U-1 true post-sign-out Login screen** — Sign out from the demo session, reach the actual operator-web Login screen. Validate: no green logo, no "use the same forge & flow operator account..." subtitle, login form copy matches OW-0a/0b. NO additional patch required — just click Sign out + wait for redirect. (Deferred this round to preserve context.)
- [ ] **OW-4 business-scope inverse** — Requires Patch 3 (business-scoped demo session OR `primaryLocationId: null` on `kDemoOperatorWebSession`). Then verify Locations item appears in left nav at Business scope.
- [ ] **OW-8c lost-authenticator with MFA enrolled** — Requires Patch 4 (seed MFA factor in `DemoWebSecurityGateway` for `kDemoOperatorWebSession`). Then verify "Request removal" CTA + "Cancel request" copy on My account → Two-factor sign-in.
- [ ] **RP-15 manager-once cap state transitions** — Requires Patch 5 (Manager-role demo session — change `kDemoOperatorWebSession` role from `operator_owner` to a Manager role, OR add `kDemoOperatorWebManagerSession`). Then submit an override at Location scope → confirm cap UX kicks in (row read-only / cap copy / Save disabled) → second attempt blocked.
