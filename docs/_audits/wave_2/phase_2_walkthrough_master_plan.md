...
| **Admin console** | Claude2 lane | 🟡 IN PROGRESS — round 1 closed 2026-05-14 evening: drove 16 of 25 admin surfaces live (Phase A shell + header + nav, Phase B System health + AI Metrics + Support logs + Launch controls, Phase C/D Default roles + RP-9 gate + Members + Invite dialog (RP-10) + My Account (W-4) + Security/audit/sessions + Audit log, Phase E Operator detail + Timing setup + Per-location data accuracy + Polling+pricing + Plans and limits, Phase F Vendor Applicability (OW-13d 3-tab) + Connected services + V-1 rename, Phase G Knowledge base). 9 surfaces code-anchored only (admin sign-in card, Roles+Hierarchy+Sessions, Vendor connections per-location, Support workspace, Default Role Catalog publish dialog, Audited support actions modals). 4 new admin gaps filed (`AC-1-FU-admin-actor-kind-catalog-drift`, `AC-1-FU-team-session-force-logout-label`, `RP-15-FU-admin-demo-override-fixture`, `MO-5b-FU-admin-my-account`). Admin matrix appended to verification doc; dev patches reverted; tree clean. | `claude2/admin-walkthrough-phase-2` (commit pending this round) | NEXT session: drive the 9 deferred admin click-paths (admin sign-in card without skip patch + Roles+Hierarchy+Sessions Access screen + Vendor connections per-location + Support workspace + Default Role Catalog publish dialog + Audited support actions modals + top-bar scope flip + ff_support read-only spot-check on Default roles) → close lane. |
...
| `AC-1-FU-admin-actor-kind-catalog-drift` (new 2026-05-14 evening, admin lane) | Wave 2 AC-1 canonical `ActorKindLabelCatalog` is consumed ONLY by orphan `lib/admin/screens/audit_log_admin_screen.dart` (not mounted in any admin route — confirmed via grep). The LIVE admin audit-log surface inside `lib/admin/screens/audited_support_actions_admin_screen.dart:1526,1672` uses `AuditActorKind.displayLabel` from `lib/admin/services/audited_support_actions_admin_gateway.dart:96-119` with parallel labels ("Operator member" / "Service account") that drift against the catalog's ("Team member" / "Automated service") | `lib/admin/screens/audited_support_actions_admin_screen.dart` + `lib/admin/services/audited_support_actions_admin_gateway.dart` (live consumer); `lib/admin/screens/audit_log_admin_screen.dart` (orphan) | Operator-facing copy drift across the two admin audit surfaces | **FIX-LIVE candidate** (small refactor — point `AuditActorKind.displayLabel` at `ActorKindLabelCatalog.labelFor()`). Operator decision dependency on canonical label (paired with `MO-5b-FU`) | open — operator decision pending |
| `AC-1-FU-team-session-force-logout-label` (new 2026-05-14 evening, admin lane) | The `team.session.force_logout` action chip on the admin Security/audit/sessions audit-log filter UI renders the machine key as its own description (no human label). Sibling `admin.session.force_logout` renders "Forced session logout" correctly | `lib/admin/screens/audited_support_actions_admin_screen.dart` action chip catalog | Operator-visible bug (Audit log filter looks broken on that one row) | **FIX-LIVE candidate** (one-line label addition; mirror `admin.session.force_logout`'s "Forced session logout" or pick e.g. "Force-signed out a team session") | open — fix-live candidate |
...
- [ ] Admin sign-in screen — code-anchored at `lib/admin/admin_auth_gate.dart:255-273` (3 fixture users); skip-sign-in dev patch bypassed this surface
- [x] Admin top bar / operator switcher — ✅ live (F&F logo + Admin Console + Ecosystem admin role pill + identity chip + Sign out)
- [x] Admin Health (`health_admin_screen.dart`) — ✅ QI-8 3-tab tile rendering driven live (Advisor data / App service / Ecosystem)
- [x] Admin Default Role Catalog (`default_role_catalog_admin_screen.dart`) — ✅ RP-9 permission gate confirmed via super_admin path
- [ ] Admin Default Role Catalog publish dialog (`default_role_catalog_publish_dialog.dart`) — code-anchored only (needs Add role + Publish flow)
...
- [ ] Admin Audit log (`audit_log_admin_screen.dart`) — 🐛 ORPHAN — standalone widget not mounted in any admin route. Live admin audit log lives inside `audited_support_actions_admin_screen.dart`. Filed `AC-1-FU-admin-actor-kind-catalog-drift`.
- [x] Admin Audited support actions (`audited_support_actions_admin_screen.dart`) — ✅ AC-1 actor_kind labels driven live (filed drift gap); audit log + sessions + actions all rendered
- [x] Admin Vendor applicability (`vendor_applicability_admin_screen.dart`) — ✅ OW-13d 3-tab editor (Wage / Covers / Polling) driven live
- [ ] Admin Vendor connections (`vendor_connections_admin_mount.dart`) — code-anchored only (requires location selection from operator detail)
...
- [ ] Admin Roles + Hierarchy + Sessions (`roles_hierarchy_sessions_admin_screen.dart`) — code-anchored only (3-tab Access screen reached via Members → onOpenAccess)
...
- [ ] Admin Support operator view (`support_operator_view_admin_screen.dart`) — code-anchored only (reached via operator-detail onOpenSupportOperatorView)
