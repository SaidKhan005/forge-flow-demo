...
| `FU-mobile-notification-tray-inconsistent-with-locked-plan-state` | P2-03 / notifications | After cold boot, the bell-icon badge shows 1 unread notification AND the prior batch's notification capture showed "New Weekly Plan Locked / Weekly operating plan for 2026-03-23 to 2026-03-29 is now active." — but the Shift dashboard simultaneously says "LOCKED PLAN UNAVAILABLE" + "No locked weekly plan is available for the current week." The notification implies a lock exists; the dashboard says no lock exists. **Either the notification is stale-seeded (orphaned from a prior session) OR the locked-plan reader is filtering by "current week" and the seeded plan is for a different week.** Filing as inconsistency — need source-trace to determine which side is wrong. | Metric-honesty risk — the operator sees conflicting signals. Source-trace required to disposition. |
