...
| Slice | PR | Live state | Evidence |
|---|---|---|---|
| W-1 Members edit-user write path | #680 | 🟢 | Team members row 3-dot menu opened live; menu items = **Edit member / Suspend / Reset password / Reset two-factor sign-in / Remove from team**. The "Edit member" entry is the dialog entry point. Dialog itself not driven (modal-overlay coordinates failed); the dialog body's field set (email, name, role assign/revoke, hierarchy rotation) is code-anchored in `lib/operator_web/screens/edit_member_dialog.dart`. Operator spot-check recommended. |
| W-1-FU Role + hierarchy rotation in edit-member dialog | #689 | 🟢 | Inherits the W-1 dialog state. Not driven; code-anchored. |
...
| RP-15 Benchmark override cap (operator-web side) | #733 | 🟢 | Benchmarks screen visible with hierarchy tree (Demo Restaurant Group → Demo Bistro → East Region {Downtown, North Loop} + West Region {Riverside}), CPLH / SPLH / PPA tabs (CPLH selected), per-node target-cycle values (all 4.58), override card with Override value field + Save button + trash icon. Selected scope: Demo Restaurant Group. Effective CPLH is 4.58 from Target cycle. **Cap behavior + admin-undo path NOT exercised** (no override actually submitted) — Manager-once cap UX state transitions and the admin-undo controls remain spot-check territory for the admin lane. |
...
| ID (suggested) | Title | Where | Impact |
|---|---|---|---|
| `R-2L-FU-demo-fixture` | Port v2 catalog into operator-web demo `kDemoTeamRolesFixture` | `lib/operator_web/services/demo_team_fixtures.dart:250-305` (currently seeds v1 5-role list) | Demo-fidelity only; production reads SQL catalog from R-2L migration. Operator demos see v1 catalog instead of v2. Not V1-blocking. |
| `U-FU-hp11-account-demo-defaults` | Populate Region/Locale/rollover_hour defaults in demo business identity OR harden `HierarchyScopeNotice` interpolation against null inherited values | `lib/operator_web/services/demo_team_fixtures.dart` business identity defaults + `HierarchyScopeNotice` widget | Demo string drift: "Inherits the business default from Business: **null / null**" + "**null:00 local**" visible in production demo. Cosmetic. Not V1-blocking. |
| `MO-5b-FU-operator-web` (extend) | MO-5b "Two-factor authentication" label sweep extends to operator-web surfaces — currently the same My account Two-factor card mixes 3 labels ("Two-factor sign-in" heading, "MFA: Not enrolled" badge, "2FA" in body copy, "Enable two-factor sign-in" button), and the Audit log Action filter chips read "MFA enrolled (authenticator)" + "MFA factor removed" but also "Reset two-factor sign-in" | `lib/operator_web/screens/my_account_screen.dart` Two-factor card; `lib/operator_web/widgets/audit_log_action_filter.dart` chip set; `lib/admin/admin_human_labels.dart` (or wherever audit_kind → label mapping lives) | The original MO-5b cited 4 mobile surfaces; this pass shows operator-web has at least 2 more surfaces with the same drift. Bundle into the canonical-label sweep slice once operator picks the canonical label. Not V1-blocking. |
| `OW-6d-FU-edit-button-UX` (optional) | Operator decision: leave the 3-dot menu (current) vs replace with literal inline "Edit user" button (debug.md 156 literal ask) | `lib/operator_web/screens/team_members_screen.dart` row trailing | UX call. The dialog is shipped + reachable; the question is whether the row's primary affordance should be a single Edit button (with Suspend/Reset moved under a separate trailing menu) or the current consolidated 3-dot. |
| `OW-4-FU-business-scope-demo` (optional) | Demo session scope-flip support so the business-scope inverse of OW-4 (Locations item appears in nav) can be exercised live | `lib/operator_web/auth/operator_web_auth_source.dart` `kDemoOperatorWebSession` is location-pinned; need a `kDemoOperatorWebBusinessSession` or a scope-toggle in the demo gateway | Demo-only walkthrough nicety. Production sessions can flip scope; demo can't. Not V1-blocking. |
