...
| 01 | **Drawer (business scope picker)** — ☰ tap | 🟢 FIX MERGED (live re-drive pending) | `p1_06_drawer_open.png` (capture before tap; drawer didn't respond to edge-swipe `input swipe 0 1200 700 1200 250`, only to explicit hamburger tap) + `p1_07_business_scope_picker.png` (drawer open, pre-fix empty). **Empty-state fix shipped via [PR #755](https://github.com/SaidKhan005/forge-flow-demo/pull/755) (squash-merged 2026-05-15 03:31 UTC as master `871bf60e`)** — Background-Agent A worker slice `Mobile-FU-business-scope-drawer-seed`, audited in [docs/_audits/wave_2/pr_755_mobile_drawer_seed.md](docs/_audits/wave_2/pr_755_mobile_drawer_seed.md). Root cause: `RestaurantScopeNotifier._availableScopes` was never populated by demo bootstrap because `loadBusinessScopes(client:)` is gated on a `BusinessScopeClient` provider that demo leaves null. Fix: new `seedAvailableScopesFromLocal()` at [lib/state/restaurant_scope_notifier.dart:78](lib/state/restaurant_scope_notifier.dart:78) hydrates `_availableScopes` from the locally-seeded `restaurant_locations` rows (same rows the demo writer side seeds, HP #2 compliant — no `kDemoMode` reader carve-out added). Call-site fallback at [lib/forge_flow_app.dart:819](lib/forge_flow_app.dart:819) fires the seed when `_resolveBusinessScopeClient()` returns null, mirroring the network path's loading-state keys. Drawer tile extracted as `BusinessScopeDrawerTile` (public + testable) at [lib/forge_flow_app.dart:1601+](lib/forge_flow_app.dart:1601). 14 tests pass + analyze clean per worker disclosure; 14-lens orchestrator audit verdict approve-for-merge. **Live re-drive of surface 01 deferred to next batch** — requires `flutter run` rebuild + reinstall (in flight, background task `bnqkopvh6`); expected outcome: drawer renders "Barrio Legado" as a highlighted active-scope row with `Icons.check_circle` trailing icon. **Outstanding parking**: drawer title `'Locations'` vs hamburger `content-desc="Business"` UX label decision filed separately (not in scope of PR #755). |
...
### Pass 1 — surfaces deferred (next batch)
...
|---|---|---|---|
| `Mobile-FU-business-scope-drawer-empty-in-demo` | 01 — drawer | **✅ RESOLVED via [PR #755](https://github.com/SaidKhan005/forge-flow-demo/pull/755) merged 2026-05-15 03:31 UTC (master `871bf60e`)** — sub-issue (a) fixed: `seedAvailableScopesFromLocal()` populates `_availableScopes` from `restaurant_locations` rows. Active-scope highlight preserved verbatim in the new `BusinessScopeDrawerTile`. Sub-issues (b) drawer title vs inventory phrasing + (c) hamburger a11y label vs drawer title intentionally **carved out** to a separate UX-decision parked row (filed as `Mobile-FU-business-scope-drawer-label-vs-a11y` below). Audit anchor: [docs/_audits/wave_2/pr_755_mobile_drawer_seed.md](docs/_audits/wave_2/pr_755_mobile_drawer_seed.md). | Resolved (a). (b)+(c) parked as UX decision. |
