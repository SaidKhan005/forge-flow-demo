...
| ID | Surface | Why it's a gap | Severity |
|---|---|---|---|
| `FU-mobile-cold-boot-shift-header-missing` | P2-01/05 | The Shift dashboard cold-boot empty state hides the restaurant header (no "Barrio Legado" label, no business-date row, no Live pill) even though the drawer + Variance prove the operator IS scoped to a known location with seeded data. This is jarring — the operator's "where am I" affordance disappears the moment they have no locked plan, even though identity context exists. Recommend rendering the header band identically on the empty state + adding a CTA "Lock this week's plan" that takes the operator to the Plan tab. | UX. Not V1-blocking if "lock your plan" is a known day-1 setup step, but the missing header is unnecessary friction. |
