...
| 01 | **Drawer (business scope picker)** — ☰ tap | ✅ DONE-LIVE (PR #756 salvage closed the live gap from PR #755) | `p1_06_drawer_open.png` (capture before tap; drawer didn't respond to edge-swipe `input swipe 0 1200 700 1200 250`, only to explicit hamburger tap) + `p1_07_business_scope_picker.png` (drawer open, pre-fix empty). **Empty-state fix shipped via [PR #755](https://github.com/SaidKhan005/forge-flow-demo/pull/755) (squash-merged 2026-05-15 03:31 UTC as master `871bf60e`)** — Background-Agent A worker slice `Mobile-FU-business-scope-drawer-seed`, audited in [docs/_audits/wave_2/pr_755_mobile_drawer_seed.md](docs/_audits/wave_2/pr_755_mobile_drawer_seed.md). Root cause: `RestaurantScopeNotifier._availableScopes` was never populated by demo bootstrap because `loadBusinessScopes(client:)` is gated on a `BusinessScopeClient` provider that demo leaves null. Fix: new `seedAvailableScopesFromLocal()` at [lib/state/restaurant_scope_notifier.dart:78](lib/state/restaurant_scope_notifier.dart:78) hydrates `_availableScopes` from the locally-seeded `restaurant_locations` rows (same rows the demo writer side seeds, HP #2 compliant — no `kDemoMode` reader carve-out added). Call-site fallback at [lib/forge_flow_app.dart:819](lib/forge_flow_app.dart:819) fires the seed when `_resolveBusinessScopeClient()` returns null, mirroring the network path's loading-state keys. Drawer tile extracted as `BusinessScopeDrawerTile` (public + testable) at [lib/forge_flow_app.dart:1601+](lib/forge_flow_app.dart:1601). 14 tests pass + analyze clean per worker disclosure; 14-lens orchestrator audit verdict approve-for-merge. **Live re-drive completed 2026-05-15 01:05** — `p1_17_drawer_post_pr755.png` — and **the drawer is STILL EMPTY**. Sequence after merging master into the lane worktree + running `flutter run --flavor forgeflow --dart-define=kDemoMode=true`: APK rebuilt (`build/app/outputs/flutter-apk/app-forgeflow-debug.apk` timestamp `01:03`, post-merge), reinstalled to emulator-5554 (`dumpsys package com.forgeflow.app` lastUpdateTime=`2026-05-15 01:03:51`, new PID `10289`), Dart VM Service attached (`05-15 01:04:13.303 I/flutter (10289): The Dart VM service is listening on http://127.0.0.1:41383/`). Tapped hamburger → drawer opened → still rendering the pre-fix empty-state "Your available locations will appear here." **Triage finding** — confirmed the SQLite row exists on the device (pulled `/data/data/com.forgeflow.app/databases/forge_flow_v2.db` via `run-as` + `exec-out`, queried via Python sqlite3 module: 1 row `('demo_restaurant_001', 'Barrio Legado', 'America/St_Johns')`). So `listRestaurants()` would return non-empty IF called. **Root-cause hypothesis** — the call-site at [lib/forge_flow_app.dart:820](lib/forge_flow_app.dart:820) guards the seed behind `if (session == null) return;`. Logcat from the post-fix process shows `AuthSessionNotifier.rehydrate storage error: 9.3 scaffold: real SecureSessionStorage is not wired — bind flutter_secure_storage (Keychain / Android Keystore) in the app bootstrap before serving authenticated traffic.` (a known "9.3 scaffold" diagnostic). This suggests `AuthSessionNotifier.session` may be null when `addPostFrameCallback((_) => _loadBusinessScopesIfNeeded(businessScopeSession))` fires at [forge_flow_app.dart:1315](lib/forge_flow_app.dart:1315) on cold boot in demo, even though the Shift dashboard renders content (the dashboard reads `RestaurantScopeNotifier._restaurant` — a SEPARATE seed path — not `AuthSessionNotifier.session`). The worker's tests verified `seedAvailableScopesFromLocal` directly; they did not exercise the call-site `if (session == null)` gate in demo bootstrap. **Audit miss admitted** — `docs/_audits/wave_2/pr_755_mobile_drawer_seed.md` Lens 4 marked the slice ✓ pass for "Service-layer / API surface unchanged"; I did not deep-trace whether the existing `session == null` guard ALSO short-circuited the new fallback path. **Side finding**: dimmed background shows a `RenderFlex overflowed by 17 PIXELS` Android-debug indicator on a Shift card (likely the Sales/Forecast card around y=350-450 of `p1_17`); filed as `FU-mobile-shift-card-overflow-17px` below. **Live re-drive after PR #756 salvage 2026-05-15 01:49** — `p1_28_drawer_post_pr756_salvage.png` — **DRAWER NOW POPULATES**. The single demo location row renders exactly as the BusinessScopeDrawerTile design intends: terracotta storefront leading icon + bold "Barrio Legado" label (w700) + "Location" gray subtitle (w500) + terracotta-filled circle with white check-mark trailing icon (selected state). Background dimmed Shift dashboard still shows the 17px overflow indicator (waiting on a separate worker for `FU-mobile-shift-card-overflow-17px`). **Resolution path**: PR #755 → live gap → orchestrator postmortem → bundled worker A2 dispatched → worker died with API image-cap error mid-task → orchestrator salvaged A2's pre-commit diff into [PR #756](https://github.com/SaidKhan005/forge-flow-demo/pull/756) → audited + merged at master `2c54abfa` → bundle worker's own emulator install (PID 11158, installed `01:42:52`) was the same code, so live verification was direct (no second rebuild needed). Salvage audit anchor: [docs/_audits/wave_2/pr_drawer_seed_followup_salvage.md](docs/_audits/wave_2/pr_drawer_seed_followup_salvage.md). |
...
### Pass 2 — cold boot + empty states (initial batch)

> **Cold-boot method**: `adb shell pm clear com.forgeflow.app` (wipes app data + SQLite without uninstalling the APK) → `adb shell monkey -p com.forgeflow.app -c android.intent.category.LAUNCHER 1` (relaunch). The currently-installed APK has PR #756's fix (via the bundle worker's earlier install + the salvage's identical code on master `2c54abfa`).

| # | Surface | Live state | Evidence + notes |
|---|---|---|---|
| P2-01/05 | **First-frame Shift dashboard** | ✅ EMPTY STATE CLEAN | `p2_01_splash_first_paint.png`. Top bar standard (☰ + monogram + 🔔 + ⚙️) but **no restaurant header row** (no "Barrio Legado", no business-date row, no Live pill). Body: **"LOCKED PLAN UNAVAILABLE"** + "No locked weekly plan is available for the current week." + "Last import: 2026-05-15T01:56:57.833788". No null-interpolation bugs visible (clean static empty-state copy, no `${var}` artifacts). Bottom nav 4 tabs present. The empty state is the intended UX: operator must lock the current week's plan before Shift outputs render (per the `WeeklyPlanSnapshot` architecture guardrail — locked plan is the comparison anchor; without one, Shift has nothing to compare against). |
| P2-Drawer | **PR #756 regression test on cold boot** | ✅ DONE-LIVE | `p2_02_drawer_cold_boot.png`. Tapped the hamburger immediately after the cold-boot Shift render. Drawer renders **"Barrio Legado"** + storefront leading icon + check-circle trailing — exactly the post-PR-756 design, proving `_seedAvailableScopesFromBootIfEmpty()` fires correctly on the cold-boot `_load()` path. The drawer works even when the Shift dashboard body is empty — because `restaurant_locations` IS populated by the cold-boot demo seed; only the locked-plan entity is missing. |
| P2-03 | **Variance tab cold boot** | ✅ DONE-LIVE | `p2_03_variance_cold_boot.png`. Variance / This Week is **fully populated** post-cold-boot: WEEK-TO-DATE vs PLAN table with Covers 630/593/**-37**, Blended Wage $19.05/$18.98/**-$0.07**, PPA $42.08/$42.17/**+$0.10**, FOH Hours 133/132/**-1**, BOH Hours 147/138/**-9**, CPLH 4.75/4.49/**-0.26 (red)**, SPLH $180/$181/**+$0.91**. So `shift_records` + `week_records` ARE seeded on cold boot — only `WeeklyPlanSnapshot` (the locked plan entity) is missing. **Notification badge appeared** on the bell icon (🔔 **1**), suggesting a notification was seeded cold-boot too. |
| P2-04 | **Plan tab cold boot** | ✅ DONE-LIVE | `p2_04_plan_cold_boot.png`. **Fully populated** — same content as warm-boot capture `p1_15`: "Weekly Operating Plan / NEXT WEEK PROJECTIONS / COVERS 1153 / SALES $48,517" + LABOR PLAN (FOH 243 / BOH 269 / Labor % 20.1% / Labor $ $9,753) + COVER FORECAST ADJUSTED BY DAY bar chart + DAY-BY-DAY PLAN table. Confirms `shift_records` + forecast data are seeded on cold boot. |
| P2-05 | **Benchmark tab cold boot** | ✅ DONE-LIVE | `p2_05_benchmark_cold_boot.png`. **Fully populated** — same content as warm-boot capture `p1_18`: "60 Day Benchmark / TOTAL COVERS LAST 60 DAYS: 10102" + CPLH RANGE & TARGET (4.28 / 5.00 / target 4.75) + GOOD OPZ RANGE pill + coaching prose + CHOOSE STAR SHIFTS CTA + DAYPART BREAKDOWN. Confirms 60-day historical data + benchmarks are seeded on cold boot. |
| P2-06 | **Settings — Setup tab cold boot** | ✅ DONE-LIVE | `p2_06_settings_setup_cold_boot.png`. **Fully populated** — same content as warm-boot capture `p1_25`. Covers setup section + Type today's covers card (scope: "Applies to: Barrio Legado" ✓) + Business timing section with timezone/week-start visible. Tab order 3 (Setup/Integrations/Data) confirmed identical to warm-boot. |
| P2-07 | **Settings — Integrations tab cold boot** | 🟢 DIFFERENT empty-state copy than warm-boot | `p2_07_settings_integrations_cold_boot.png`. Per-category cards (POS, Reservations, Labor) show **"Status not yet loaded. Pull to refresh."** + **Unknown** status pill — different from warm-boot's "Sign in and pick a location..." + "No location" pill. Demo mode card + delegate-to-operator-console link still rendered. Filed `FU-mobile-cold-boot-integrations-copy-divergence` for UX consistency. |
| P2-08 | **Notifications fullscreen cold boot** | ✅ DONE-LIVE | `p2_08_notifications_cold_boot.png`. 1 notification: "New Weekly Plan Locked / Weekly operating plan for **2026-03-23 to 2026-03-29** is now active. / 10m ago". Header + X close + mark-all-as-read affordances identical to warm-boot capture `p1_08`. **Resolves the earlier inconsistency finding** — the seeded notification is for a week 7+ weeks in the past relative to current date 2026-05-15; the Shift dashboard's "no plan locked for current week" message is honest given that the seeded plan does not cover the current week. Both sides correct; downgraded to UX nit. |

### Pass 2 net result

**8 cold-boot surfaces captured** (P2-01 + P2-Drawer + P2-03 through P2-08). **3 UX findings** filed (cold-boot Shift header missing, Integrations copy divergence, notification-vs-dashboard UX nit). **No HP #11 null-interpolation bugs** found in any cold-boot empty state — empty-state copy is clean static text across the board. **PR #756's drawer-seed fix verified live on cold boot**. P2-02/03/04 (Welcome/Login/MFA) confirmed STUB — demo flavor builds with `requireAuth: false`. Notification tray correctly surfaces a historical "plan locked" event; Shift dashboard correctly distinguishes between historical and current-week plan state.

### Gaps filed during Pass 2 batch 1

| ID | Surface | Why it's a gap | Severity |
|---|---|---|---|
| `FU-mobile-cold-boot-shift-header-missing` | P2-01/05 | The Shift dashboard cold-boot empty state hides the restaurant header (no "Barrio Legado" label, no business-date row, no Live pill) even though the drawer + Variance prove the operator IS scoped to a known location with seeded data. **2026-05-15 02:28 update**: this is actually downstream of a more serious bug — `FU-mobile-cold-boot-shift-dashboard-stale-state` (below). Verified by tapping Settings → Data → "Move demo date forward" once (Mar 27 → Mar 28); Shift dashboard IMMEDIATELY populated with full header band, daypart chips, and metric cards, no rebuild required. So the header isn't *removed* by the empty state — the empty state is itself a wrong render that should not have fired. Demoting this row from "render the header in empty state" to "fix the empty state so it doesn't render incorrectly in the first place". | Demoted in priority — fix the root cause (parent row below) and this becomes moot. |
| `FU-mobile-cold-boot-shift-dashboard-stale-state` | P2-01/05 + P2-Variance | **NEW + MOST IMPORTANT FINDING**. The cold-boot Shift dashboard renders the "LOCKED PLAN UNAVAILABLE" empty state **even when the SQLite DB has all the required data**. Verified by pulling `/data/data/com.forgeflow.app/databases/forge_flow_v2.db` after `pm clear` + relaunch + capture and querying via Python sqlite3: `mock_replay_state=('demo_restaurant_001', '2026-03-27')`, `weekly_plan_snapshots`=1 row with `week_key='2026-03-23_2026-03-29'`, `open_shift_snapshots`=6 rows including 1 with status='open' for `business_date='2026-03-27'` `day_label='Fri'`, `shift_records`=126, `target_cycles`=1. Trace through `lib/state/shift_dashboard_notifier.dart:89-178` shows the path SHOULD find the locked plan (`businessDate`→`'2026-03-27'`→`weekKey='2026-03-23_2026-03-29'`→snapshot exists→projector→dayPlan match on 'Fri'), but the live render shows the empty-state path (line 166-167 `loadedReadModel = null; lockedPlanUnavailable = true`) was taken. **Conclusive verification**: tapping Settings → Data → "Move demo date forward" once (which calls `ShiftService.instance.advanceMockReplayDay()` + `onAfterWrite()` reload) immediately makes the dashboard populate (capture `p2_diag_shift_after_advance.png`, Sat Mar 28 with full data). So the data path works fine post-reload; the cold-boot first-load either races the seed completion OR the projector silently returns no dayPlan for the cold-boot dayLabel. **Demo-data only** — the entire `MockIntegrationReplaySeed` + `mock_replay_state` + `advanceMockReplayDay` chain is in `lib/dev/` (dev-only per Service-Layer Split) and writer-side only. Reader paths do NOT branch on `kDemoMode`. Fix will live in the demo writer side, preserving HP #2. | **HIGH priority — V1 demo onboarding blocker**. New operators (or anyone who reinstalls / wipes data) see a broken-looking Shift dashboard on first boot. Needs root-cause + fix before V1. |
| `FU-mobile-notification-tray-inconsistent-with-locked-plan-state` | P2-03 / notifications | **DOWNGRADED to UX nit, NOT a metric-honesty bug.** Re-reading the notification on cold-boot (`p2_08_notifications_cold_boot.png`): "New Weekly Plan Locked / Weekly operating plan for **2026-03-23 to 2026-03-29** is now active." Current date is 2026-05-15 — the seeded plan is for a week **7+ weeks in the past**. Both sides are honest: notifications correctly shows the historical event (a plan was locked once), Shift dashboard correctly says "no plan locked for **THE CURRENT WEEK**". The badge giving operator the impression of "live state" rather than "historical event" is a soft UX nit but not a bug. | UX-only nit. Not V1-blocking. Notifications correctly preserve history; operator just needs to read the date range. |
| `FU-mobile-cold-boot-integrations-copy-divergence` | P2-07 / Integrations | The Integrations tab shows **different empty-state copy** on cold boot vs warm boot. Cold-boot: "Status not yet loaded. Pull to refresh." + **Unknown** status pill (`p2_07_settings_integrations_cold_boot.png`). Warm-boot: "Sign in and pick a location to see this category's status." + **No location** pill (`p1_26_settings_integrations.png`). Both states are honest for their respective conditions — cold-boot is genuinely "data not loaded yet", warm-boot is "category gated on location pick" — but the copy divergence may confuse operators who switch between the two states via background app kill / restart. **Recommend**: unify the empty-state copy across both paths OR document the two states distinctly in the inline rationale. | Low. UX consistency. Not V1-blocking. |
...
| ID | Surface | Why it's a gap | Severity |
|---|---|---|---|
| `Mobile-FU-business-scope-drawer-empty-in-demo` | 01 — drawer | **✅ RESOLVED 2026-05-15 04:18 UTC** via [PR #756](https://github.com/SaidKhan005/forge-flow-demo/pull/756) (squash-merged at master `2c54abfa`). Live verification at 01:49 NDT confirmed the drawer populates with "Barrio Legado" + active-row check icon (`p1_28_drawer_post_pr756_salvage.png`). PR #756 was a salvage of dead-worker A2's pre-commit diff after the worker hit an API-image-cap error during their own live-verify step. The fix hooks `_seedAvailableScopesFromBootIfEmpty()` into the session-independent `_load()` path that already populates `_restaurant`, with an `_kBootFallbackOperatorId = 'demo-operator'` sentinel that production overwrites within milliseconds. Audit anchor: [docs/_audits/wave_2/pr_drawer_seed_followup_salvage.md](docs/_audits/wave_2/pr_drawer_seed_followup_salvage.md). | RESOLVED. Lesson: PR #755 audit's miss (Lens 4 didn't trace the call-site `session == null` gate) is the audit anti-pattern the salvage's new integration tests now guard against. The new test "default constructor hydrates availableScopes from the seeded restaurant_locations rows so the drawer is non-empty in demo" is the regression fence for this class of bug. |
...
| `FU-mobile-shift-card-overflow-17px` | 00 — boot baseline (Shift dashboard) | **✅ RESOLVED 2026-05-15 04:35 UTC** via [PR #757](https://github.com/SaidKhan005/forge-flow-demo/pull/757) merged at master `ba345300`. Worker A3 root-cause: `MetricPill._buildEmptyState` + `_buildUnavailableState` rendered the optional tooltip as bare `Text` with no `maxLines` cap. SHIFT OUTPUTS hosts pills inside `IntrinsicHeight + Row + Expanded`; the intrinsic-height pass measures the tooltip at unbounded width (1 line) but real layout at ~186dp wraps to 2 lines, producing the ~17px overflow. Fix: wrap tooltip Text in `Flexible(child: Text(..., maxLines: 2, overflow: TextOverflow.ellipsis))` in both render paths at [lib/widgets/metric_pill.dart:264-274 + :292-302](lib/widgets/metric_pill.dart:264). 2 new widget regression guards (forward-looking; worker honestly disclosed the widget-test fallback-font caveat). Audit anchor: [docs/_audits/wave_2/pr_757_shift_card_overflow.md](docs/_audits/wave_2/pr_757_shift_card_overflow.md). **Live verify deferred** to next Pass 2 batch's fresh-build cycle. | RESOLVED. |
