...
- **AWAITING OPERATOR MERGE APPROVAL (auth-touching gate):** G71 → PR `#912` built + independently audited **APPROVE-FOR-MERGE** (single 401→refresh-once→retry chokepoint in `admin_http_timeout.dart`; idempotency-key + body preserved verbatim on retry; demo/share-preview byte-equivalent; 3 files). Auth-touching ⇒ explicit operator merge approval required before merge.
