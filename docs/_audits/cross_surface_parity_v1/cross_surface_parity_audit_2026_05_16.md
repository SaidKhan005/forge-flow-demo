...
- **Backlog wave 2026-05-17 (operator-selected subset) — CLOSED/MERGED:** G73 → `#911` (`3f61e8be`, admin demo v1-role-seed carve-out documented; demo_mode_contract bullet; zero behavior change). OW-G73 + OW-G74 → `#913` (`e1c59ecd`, dead `kOperatorWebNavWageAuthority` route + false comments removed; init-failure raw stack-trace no longer rendered, `FlutterError.reportError` added so engineers keep the detail, fail-closed preserved). X-G71 → `#914` (`e004616e`, new admin notification-preferences screen; reuses the existing bearer-scoped `/v1/operator/notification-preferences` route — no proxy/backend change, non-auth). All independently audited APPROVE-FOR-MERGE, behavior-neutral/additive.
- **CLOSED/MERGED (auth-touching, operator-approved):** G71 → `#912` (`6e47ec56`, single 401→refresh-once→retry chokepoint in `admin_http_timeout.dart`; retry reuses same Idempotency-Key + body verbatim; demo/share-preview byte-equivalent). Independently audited APPROVE; operator approved; rebased once (additive conflict with #914 in `main_admin.dart`, both sides kept, G71 footprint unchanged, re-verified 9/9 + 29/29 green) then merged.
- **STILL OPEN, recorded, not started (operator deferred — non-mobile MED):** G72 (admin demo-tell), X-G70 (admin demo-tell parity leg only). The operator selected the rest of the §0b backlog; these two remain by operator choice.
- **Doc loose ends status:** `admin_shell_widget_test` "side nav lists primary routes" IA-drift now quarantined in `docs/KNOWN_FAILING_TESTS.md` (2026-05-17). Stale `stash@{0}` — **NOT dropped**: re-inspection showed `stash@{0}` is an `autostash` holding operator-protected Phase-7.58 WIP (`labor_model.dart` +173, `shift_service.dart`, `lever_logic_test.dart`, etc.), NOT the redundant admin_security_gateway stash the earlier handoff assumed (stash indices shifted). Left untouched; flagged to operator.
- A minor non-blocking nit from #913 audit (N1): two now-dangling comments in `operator_web_router.dart:1241-1242,1272-1273` still reference the deleted "standalone Wage authority case"; behavior unaffected; trivial follow-up.
### End-to-end wiring verification — 2026-05-17 (3 read-only agents traced every merged item def→DI→surface/route→runtime)

Operator asked to confirm nothing shipped this wave is skeleton/orphan code. Verdicts on `origin/master`:
- **WIRED-LIVE, no orphans:** G7-admin + G7-opweb (v2 role constants consumed by real gates; phantom `operator_admin` gone from the live admit path; one inert `roleOperatorAdmin` string survives only in a notification-audience filter that can never match — dead-string cleanup, not a phantom), HP#11 S1 + S3 (op-web timing fully wired + nav-reachable), X-G71 (admin notif-prefs screen wired in prod DI + nav), G71 (token-refresh hook actually assigned in prod `main_admin.dart`, gated to live source — avoided the MOB-G70 trap), OW-G73/OW-G74, Round-2 OW-G72/OW-G70 (live on real save paths; OW-G70 scope corrected to 3 routes + wage-authority-via-data-accuracy by-design after OW-G73, not 4 standalone).
- **Defects found + FIXED (production-DI hop missing in `main_admin.dart` — code-complete but inert in a live admin session):**
  - **HP#11 S2/S4** — `HttpAdminBusinessTimingResolutionGateway` was never instantiated in prod ⇒ admin Timing screens rendered the empty in-memory state, never called the merged S2 cross-tenant route. **FIXED → `#923`** (`cf7af932`, adds the prod resolver mirroring the X-G71 pattern; demo/share-preview byte-equivalent; independently audited APPROVE; operator-approved auth/proxy gate). HP#11 S2/S4 now genuinely functional for a live super_admin/ff_support.
  - **G70 surface** — the admin "My Account → edit identity" surface was unwired in prod (`adminAccountGateway` never passed ⇒ affordance disabled), so G70's caller-stable idempotency key never ran live. Pre-existing (G70 hardened an already-unconnected surface). **FIXED → `#925`** (`0171f05a`, same prod-DI mirror; G70 idempotency logic untouched; demo byte-equivalent; audited APPROVE; operator-approved auth/proxy gate). G70's stable key now runs on the live `PATCH /v1/auth/self/profile`.
- **Net:** every cross-surface-parity item merged this wave is now verified wired end-to-end in the production path. No remaining skeleton/orphan code in scope.
