...
-- AGE projection is generated by `tool/advisor_corpus prepare-age-projection`
-- and remains conditional on AGE availability, so missing AGE should not block
-- loading the staging rows.
do $$
begin
  if exists (
    select 1
    ...
    raise notice 'Apache AGE extension is not available; run tool/advisor_corpus prepare-age-projection to generate the projection artifacts and apply them once AGE is provisioned.';
  end if;
end;
$$;
...
comment on table public.advisor_graph_node_seeds is
  '11a graph node staging records projected into Apache AGE by tool/advisor_corpus prepare-age-projection (conditional on AGE availability).';
comment on table public.advisor_graph_edge_hints is
  '11a graph edge staging records projected into Apache AGE by tool/advisor_corpus prepare-age-projection (conditional on AGE availability).';
...
comment on column public.advisor_source_chunks.active is
  '11a.11a active/inactive flag. True for chunks emitted by the most recent materialization of their doc_id; false for prior content-addressed chunks the new run replaced. Old chunks are never deleted so old advisor recommendations remain replayable against the exact chunk text they cited. advisor_search_chunks filters to active = true.';
