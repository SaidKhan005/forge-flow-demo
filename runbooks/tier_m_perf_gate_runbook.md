...
so the live run is BLOCKED on preflight today. The current runner
requires the Production1 corpus seed/harness artifacts generated by
`cutover.1`; this gate blocks first-operator/live-traffic slices,
not the corpus-load setup work.
...
Until cleared, first-operator/live-traffic slices (`cutover.2+`) do
not start. `cutover.1` production corpus load is the prerequisite that
generates this runbook's seed and harness artifacts.
...
This is consistent with the cutover plan: cutover.0b cannot run
