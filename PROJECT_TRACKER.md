...
- **Production1 runtime live** (2026-05-06). Cloud Run + Firebase + Postgres-CMK + production DNS for `app.forgeflow.app` and `mail.forgeflow.app` all provisioned and verified. 10 of 12 follow-up migrations staging-verified; 2 remaining need operator decision/approval before apply. The single remaining production-domain switch is the Firebase Auth action surface (`auth.feflow.org` from `forge-flow-staging.web.app` to `forge-flow-production1.web.app` + `callbackUri = https://auth.feflow.org/auth/action` + 4 validation checks). Resume guide: `docs/_execution/2026-05-06_v1_operator_punchlist_execution.md`. Apply queue + decision details tracked in `docs/POST_HARDENING_FOLLOWUPS.md` "P0 - Production1 Migration Apply Gap" + runbook `runbooks/phase_9_production1_migration_apply_runbook.md`.
