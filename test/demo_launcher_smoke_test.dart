...
void main() {
  group('Demo launcher dart-defines stay aligned with Dart-side flag names', () {
    test('run_flutter_dev.ps1 demo emits kDemoMode + FORGE_FLOW_DEMO_MODE; '
    ...
    test(
      ...
      () {
        ...
        expect(
          mainOperatorWeb,
          matches(
            RegExp(r"bool\.fromEnvironment\(\s*'OPERATOR_WEB_DEMO_AUTH'"),
          ),
          reason:
        ...
        expect(
          mainOperatorWeb,
          matches(
            RegExp(r"String\.fromEnvironment\(\s*'OPERATOR_WEB_DEMO_SCENARIO'"),
          ),
          reason:
              'lib/main_operator_web.dart no longer reads '
        ...
        expect(
          mainOperatorWeb,
          contains('DemoOperatorWebAccountGateway'),
          reason:
              'Normal operator-web -Mode demo must expose the Account write '
              'gateway so Location timezone and My account edit layers are '
              'reachable without live mutations.',
        );
        expect(
          mainOperatorWeb,
        ...
        expect(
          ...
          reason:
              ...
              'gateway so Edit Time Settings opens the real editor instead '
              'of a read-only reminder.',
        );
        expect(
          mainOperatorWeb,
          matches(
            RegExp(
              r'HttpBusinessTimingReadGateway\(\s*'
              r'gateway:\s*businessTimingWriteGateway,\s*\)',
            ),
          ),
          reason:
              'The demo timing read page must be backed by the same '
              'in-memory timing gateway as the editor, so the page does not '
              'say read-only while the edit layer is actually reachable.',
        );
      },
    );

    test('run_admin_console_dev.ps1 demo emits ADMIN_SHARE_PREVIEW + '
      ...
      expect(
        mainAdmin,
        matches(
          RegExp(r"bool\.fromEnvironment\(\s*'ADMIN_SHARE_PREVIEW'\s*\)"),
        ),
        reason: 'lib/main_admin.dart no longer reads ADMIN_SHARE_PREVIEW.',
      );
      expect(
        mainAdmin,
        matches(
          RegExp(
            r"bool\.fromEnvironment\(\s*'ADMIN_SHARE_PREVIEW_AS_SUPER_ADMIN'",
          ),
        ),
        reason:
  ...
  group('Admin gateway resolvers honor share-preview', () {
    test('every `if (_kAdminDemoAuth) return null;` site in lib/main_admin.dart '
  ...
  group('Shared dev-CSP swap helper stays compatible with web/index.html', () {
    test("the regex in scripts/_dev_csp_swap.ps1 still matches the CSP <meta> "
    ...
    test("the dev CSP heredoc in scripts/_dev_csp_swap.ps1 keeps Flutter's "
  ...
  group('Web console launchers default to -Device chrome (auto-spawn)', () {
    test(
      ...
      () {
        ...
        expect(
          script,
          matches(
            RegExp(r"\[string\]\s+\$Device\s*=\s*'chrome'", multiLine: true),
          ),
          reason:
    ...
    test('run_admin_console_dev.ps1 default -Device is chrome', () {
      final script = _read('scripts/run_admin_console_dev.ps1');
      expect(
        script,
        matches(
  ...
  group('Production-gate refusal without -IUnderstand', () {
    test('all three launchers contain the BLOCKED: -Mode production '
        'targets message', () {
  ...
  group('Splash-removal fallback (PR #809)', () {
    test('forge_flow_web_bootstrap.js installs a MutationObserver that calls '
  ...
  group('README master matrix points at the live launcher scripts', () {
    test('README contains the README master matrix demo commands', () {
