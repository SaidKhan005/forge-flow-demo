...
//   * rows read "Never" when no event has stamped that table
...
void main() {
  group('SettingsDataFreshnessSection', () {
    testWidgets(
      'renders one row per shared-state table; all read "Never" when '
      ...
      (tester) async {
        ...
        for (final label in <String>[
          ...
          'target_cycle_provenance',
          'app_notifications',
          'audit_trail',
          'connector_configs',
          'restaurant_users + roles + role_permissions',
        ]) {
          expect(
        ...
        // 8 rows × at least one "Never" each.
        expect(find.text('Never'), findsNWidgets(8));
      },
    );

    testWidgets(
      'after ingesting a shared_state frame for a table, that row '
      'flips from "Never" to a relative timestamp and tooltip shows '
      'the UTC ISO',
      (tester) async {
        ...
        expect(
          find.descendant(
            of: find.byKey(
              const Key(
                'settings_data_freshness_row_weekly_plan_snapshots',
              ),
            ),
        ...
        // Other rows still read "Never".
        expect(
          find.descendant(
            of: find.byKey(
              const Key('settings_data_freshness_row_restaurants'),
            ),
            matching: find.text('Never'),
          ),
          findsOneWidget,
    ...
    testWidgets(
      'restaurant_users + roles + role_permissions row updates when '
      'ANY of the three Phase 9 tables receives a frame; row reflects '
      'the newest of the three timestamps',
      (tester) async {
        var tick = 0;
