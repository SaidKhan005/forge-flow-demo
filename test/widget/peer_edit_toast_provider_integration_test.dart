...
//     row for that table flips from "No updates yet" to a relative timestamp
//
...
void main() {
  testWidgets('bus.publish drives both PeerEditToast and '
          ...
          home: Builder(
            builder: (context) {
              ...
              return PeerEditToast(
                ...
                child: const Scaffold(body: SettingsDataFreshnessSection()),
              );
            },
          ),
        ),
    ...
    // Pre-publish: every freshness row reads "No updates yet"; no toast.
    ...
    // "No updates yet". Seven other rows still do.
    expect(
      find.descendant(
        of: find.byKey(const Key('settings_data_freshness_row_Audit history')),
        matching: find.text('No updates yet'),
      ),
      findsNothing,
    );
    expect(find.text('No updates yet'), findsNWidgets(7));
