...
void main() {
  ...
  setUp(() async {
    ...
    final fixtureRoot = Directory(
      p.join(Directory.current.path, 'test', 'fixtures', 'graphify_v5_sample'),
    );
  ...
  test(
    'drops candidates whose source_file is not in the manifest scope',
    () async {
      ...
      expect(
        result.droppedOutOfScopeCount,
        greaterThanOrEqualTo(1),
        reason:
            'fixture seeds at least one out-of-scope node which the '
  ...
  test('writes node + edge JSONL files at the expected paths', () async {
    ...
    expect(
      manifestSummary['corpus_manifest_path'],
      equals('docs/Knowledge_graph_docs/corpus_manifest.yaml'),
      reason:
          'generated manifests must be repo-relative so packaged '
          'Cloud Run artifacts do not leak local workstation paths',
    );
    ...
    expect(
      ...
      equals(5),
      reason:
          'fixture: 2 pairwise edges + 3 hyperedge fan-out pairs '
          '(C(3,2) = 3) = 5 distinct edges after dedupe',
    );
  });

  test('preserves the producer confidence label string verbatim', () async {
    final manifest = await loadStagedManifest();
  ...
  test('preserves the numeric confidence_score on every node + edge', () async {
    final manifest = await loadStagedManifest();
    final importer = GraphifyCandidateImporter(repoRoot: tempRepo);
    final result = await importer.prepare(manifest: manifest);
    ...
    for (final row in nodeRows) {
      expect(
        ...
        isA<num>(),
        reason:
            'node ${row['candidate_key']} should carry a numeric '
            'confidence_score',
      );
    }
  });
  ...
  test(
    'low-confidence node is emitted with score < 0.7 in the JSONL',
    () async {
      final manifest = await loadStagedManifest();
      final importer = GraphifyCandidateImporter(repoRoot: tempRepo);
      final result = await importer.prepare(manifest: manifest);
      ...
      final nodeRows =
          File(p.join(result.outputDirectory, graphifyNodeCandidatesFileName))
              .readAsLinesSync()
              .where((l) => l.trim().isNotEmpty)
              .map((l) => jsonDecode(l) as Map<String, Object?>)
              .toList();
      ...
      expect(
        ...
        isNotEmpty,
        reason:
            'fixture seeds weekly_plan_concept at confidence_score '
  ...
  test(
    'hyperedges fan out to pairwise edges with shared hyperedge_id',
    () async {
      final manifest = await loadStagedManifest();
      final importer = GraphifyCandidateImporter(repoRoot: tempRepo);
      final result = await importer.prepare(manifest: manifest);
      ...
      final edgeRows =
          File(p.join(result.outputDirectory, graphifyEdgeCandidatesFileName))
              .readAsLinesSync()
              .where((l) => l.trim().isNotEmpty)
              .map((l) => jsonDecode(l) as Map<String, Object?>)
              .toList();
      ...
      expect(
        ...
        hasLength(3),
        reason:
            'fixture has one hyperedge with arity 3 → C(3,2) = 3 '
      ...
      expect(
        ...
        hasLength(1),
        reason:
            'every fan-out from the same hyperedge must share the '
  ...
  test(
    'second invocation produces byte-identical output (deterministic)',
    () async {
      final manifest = await loadStagedManifest();
      ...
      expect(
        ...
        equals(nodes1),
        reason:
            'rerunning the importer against unchanged inputs must '
  ...
  test('advisor proxy image carries sanitized candidate artifacts', () {
    ...
    for (final name in <String>[
      ...
      final file = File(p.join(artifactRoot.path, name));
      expect(file.existsSync(), isTrue, reason: '$name must ship with proxy');
    ...
    final manifest =
        jsonDecode(
              File(
                p.join(artifactRoot.path, graphifyCandidateManifestFileName),
              ).readAsStringSync(),
            )
    ...
    expect(
      dockerfile,
      contains('/app/graphify-out'),
      reason:
          'RepositoryGraphCandidatesProxyGateway reads /app/graphify-out on '
          'Cloud Run because Directory.current is /app',
    );
  });
}
