...
void main() {
  group('R7b (a) — no legacy covers-column SQL in the proxy', () {
    test(
      'proxy_bootstrap.dart contains zero SQL referencing the legacy '
      ...
      () {
        ...
        for (final col in const <String>[
          ...
          expect(
            code.contains('s.$col'),
            isFalse,
            reason: 'view-aliased legacy column $col must not be '
          ...
          expect(
            ...
            isFalse,
            reason: 'no proxy SQL may reference '
    ...
    test(
      '(d) keyed covers writes are idempotent via ON CONFLICT DO UPDATE '
  ...
  group('R7b (b)+(c) — mobile read/write wire compat + per-period', () {
    test(
      'fetchDataAccuracySettings reads the view per-period jsonb (no '
      'legacy column SELECT) and emits the legacy keys derived from it '
      'plus the per-period map (4-period operator)',
      () async {
        ...
        final selectSql = pool.lastTx!.calls
            .map((c) => c.sql)
            .firstWhere(
              (s) => s.contains('from public.data_accuracy_settings'),
            );
    ...
    test(
      'fetch emits vendor defaults for legacy keys when the keyed map '
      'has no row (unchanged default behavior)',
      () async {
        final pool = _StubPool(
          rowsByContains: <String, List<PostgresRow>>{
            'from public.data_accuracy_settings': <PostgresRow>[
    ...
    test(
      'upsertDataAccuracySettings accepts the legacy JSON keys, routes '
      'them to the keyed table (no legacy-column INSERT), and emits '
      'the legacy keys back sourced from the keyed view jsonb',
      () async {
        final pool = _StubPool(
          rowsByContains: <String, List<PostgresRow>>{
            ...
            'select (select v.covers_source_per_service_period':
                <PostgresRow>[
              <String, Object?>{
                'covers_source_per_service_period': <String, Object?>{
                  'lunch': 'forecast',
        ...
        final settingsInsert = calls
            ...
            .firstWhere(
              (s) =>
                  s.contains('insert into public.data_accuracy_settings'),
            );
        expect(settingsInsert.contains('covers_source_lunch'), isFalse);
        expect(settingsInsert.contains('covers_source_dinner'), isFalse);
        expect(
          settingsInsert.contains('covers_source_late_night'),
          isFalse,
        );

        // Three keyed-table writes, one per legacy daypart, all with
        // the 1970-01-01 sentinel + idempotent ON CONFLICT.
        ...
        final byPeriod = <Object?, Object?>{
          for (final w in keyedWrites)
            w.parameters['service_period_key']:
                w.parameters['covers_source'],
        };
        expect(byPeriod['lunch'], 'forecast');
        expect(byPeriod['dinner'], 'manual');
        ...
        expect(data['covers_source_dinner'], 'manual');
        expect(data['covers_source_late_night'], 'vendor');
        expect(
  ...
  group('R7b (c) — admin org-unit scoped override → per-period jsonb', () {
    test(
      'overrideDataAccuracyScope writes the supplied dayparts into the '
      ...
      () async {
        ...
        expect(
          scopedInsert.contains('covers_source_late_night'),
          isFalse,
        );
        ...
        // Only the two supplied dayparts go into the jsonb param;
        // late_night (null) is omitted so its existing scoped value
        // is preserved by the `existing || new` merge.
        expect(
          scopedCall.parameters['covers_per_period'],
          equals('{"lunch":"manual","dinner":"forecast"}'),
        );
        ...
        expect(settings['covers_source_lunch'], 'manual');
