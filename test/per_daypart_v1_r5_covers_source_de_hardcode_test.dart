...
void main() {
  group('R5 (a) — resolver-keyed model round-trips for 4 periods', () {
    test('fromRow projects the keyed covers_source_per_service_period map; '
      ...
      expect(settings.coversSourceFor('lunch'), equals(CoversSource.vendor));
      expect(settings.coversSourceFor('dinner'), equals(CoversSource.forecast));
      expect(
    ...
    test('manualCoversFor is keyed by service_period_id and round-trips '
    ...
    test('R7c: legacy covers_source_* columns are no longer consulted by '
    ...
    test('a keyed map wins over legacy columns when both are present', () {
      final settings = DataAccuracySettings.fromRow(<String, Object?>{
        'setting_id': 'set-1',
        'operator_id': 'op-1',
        'location_id': 'loc-1',
    ...
    test('keyed reservation_plus_walkin is preserved in settings', () {
      final settings = DataAccuracySettings.fromRow(<String, Object?>{
        'setting_id': 'set-1',
        'operator_id': 'op-1',
        'location_id': 'loc-1',
  ...
  group('R5 (b) — backfill migration preserves every legacy value', () {
    test('one INSERT maps all three legacy columns to keyed rows under '
      ...
      expect(
        sql,
        contains('insert into public.data_accuracy_service_period_settings'),
      );
      expect(
        sql,
      ...
      expect(sql, contains("('late_night', das.covers_source_late_night)"));
      // Driven off every existing settings row so no operator is
      // skipped.
      expect(sql, contains('from public.data_accuracy_settings das'));
    });

    test('backfill is data-preserving + idempotent: ON CONFLICT DO NOTHING '
    ...
    test('operator/location scoped: operator_id + location_id are copied '
    ...
    test('legacy columns are deprecated via column comments, wrapped in '
    ...
    test('migration prose is technical prose (NOT operator-facing UX copy; '
  ...
  group('R7c — fromRow with keyed/jsonb input is behavior-neutral after '
    ...
    test('keyed resolution is identical with vs without legacy columns '
      ...
      for (final period in const <String>[
        ...
        expect(
          ...
          reason:
              'period "$period" must resolve identically; '
      ...
      expect(keyedOnly.coversSourceFor('late_night'), CoversSource.manual);
      expect(keyedOnly.coversSourceFor('happy_hour'), kDefaultCoversSource);
    });
  });
}
