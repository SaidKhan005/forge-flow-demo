...
MfaRecoveryTargetRow _target({
  ...
  List<MfaRecoveryAdminRecipientRow> admins =
...
void main() {
  group('RepositoryMfaRecoveryRequestGateway', () {
    ...
    test(
      'rate limiter blocked → throws 429 with retryAfter preserved',
      () async {
    ...
    test(
      'silent accepted (queued=false) when target email is unknown',
      () async {
        final outbox = _RecordingEventOutboxRepository();
        final gateway = RepositoryMfaRecoveryRequestGateway(
          usersRepository: _RecordingUsersRepository(),
    ...
    test('silent accepted when the operator has no admin recipients', () async {
      final users = _RecordingUsersRepository(
        targetByEmail: <String, MfaRecoveryTargetRow>{
    ...
    test(
      'silent accepted when the target has no resolvable location_id',
      () async {
        final users = _RecordingUsersRepository(
          targetByEmail: <String, MfaRecoveryTargetRow>{
    ...
    test(
      'happy path enqueues outbox event with admin lists + reason',
      () async {
        final users = _RecordingUsersRepository(
          targetByEmail: <String, MfaRecoveryTargetRow>{
            'mfa@example.test': _target(
  ...
  group('InMemoryMfaRecoveryRequestRateLimiter (extra coverage)', () {
    test('per-IP rolling window allows fresh requests after rolloff', () async {
    ...
    test(
      'blank IPs share a single bucket so they cannot bypass throttles',
      () async {
        var now = DateTime.utc(2026, 4, 30, 12);
        final limiter = InMemoryMfaRecoveryRequestRateLimiter(
          now: () => now,
    ...
    test(
      'email cooldown carries retryAfter = lastAttempt + cooldown',
      () async {
        var now = DateTime.utc(2026, 4, 30, 12);
        final limiter = InMemoryMfaRecoveryRequestRateLimiter(
          now: () => now,
...
class _RecordingUsersRepository extends UsersRepository {
  _RecordingUsersRepository({Map<String, MfaRecoveryTargetRow>? targetByEmail})
