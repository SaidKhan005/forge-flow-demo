...
void main() {
  ...
  group('D — analytics computation rules', () {
    ...
    test('1 selected -> narrow with count 1', () {
      final a =
          BaselineSelectionAnalyticsService.computeAnalytics(1, [4.5]);
      expect(a.selectedShiftCount, equals(1));
      expect(a.rangeQualityLabel, equals('OPZ RANGE TOO NARROW'));
    });

    test('2 selected with width < 0.15 -> narrow', () {
      final a = BaselineSelectionAnalyticsService.computeAnalytics(
          2, [4.50, 4.60]);
  ...
  group('E — canonical path uses persisted analytics', () {
    test('canonical path returns injected analytics, not BaselineData',
        () async {
      LearnBenchmarkContextService.disableBridgeOnly();
    ...
    test('canonical path analytics differ from BaselineData values',
        () async {
      LearnBenchmarkContextService.disableBridgeOnly();
      LearnBenchmarkContextService.testCanonicalOverride = () async {
        return const LearnBenchmarkContext(
  ...
  group('F — bridge-only mode', () {
    ...
    test('analytics bridge-only returns BaselineData analytics', () async {
      ...
      final analytics =
          await BaselineSelectionAnalyticsService.instance.resolve();
      expect(analytics.selectedShiftCount,
          equals(BaselineData.selectedRecordCount));
      expect(analytics.rangeQualityLabel,
  ...
  group('J — source-aware analytics resolution', () {
    test('system_baseline returns bridge analytics, not 0/NARROW', () async {
      ...
      expect(analytics.rangeQualityLabel,
          equals(BaselineData.baselineRangeValidation.statusLabel));
      expect(analytics.rangeQualityLabel, isNot(equals('OPZ RANGE TOO NARROW')));
    });
    ...
    test('override source with testAnalyticsOverride returns injected value',
        () async {
      BaselineSelectionAnalyticsService.disableBridgeOnly();
      BaselineSelectionAnalyticsService.testAnalyticsOverride = () async {
        return const BaselineSelectionAnalytics(
  ...
  group('K — context service passes source type to analytics', () {
    test('canonical system_baseline profile does not produce 0/NARROW',
        () async {
      LearnBenchmarkContextService.disableBridgeOnly();
      BaselineSelectionAnalyticsService.disableBridgeOnly();
    ...
    test('Learn source label and targets stay on repository-backed profile',
        () async {
      LearnBenchmarkContextService.disableBridgeOnly();
      LearnBenchmarkContextService.testCanonicalOverride = () async {
        return const LearnBenchmarkContext(
  ...
  group('L — canonical path reads from persisted summary', () {
    ...
    test('canonical path with persisted summary does not use bridge analytics', () async {
      LearnBenchmarkContextService.disableBridgeOnly();
  ...
  group('N — missing-summary recovery backfill (8c1)', () {
    ...
    void setUpRecoveryScenario({
      required Future<BenchmarkSelectionSummary?> Function(String) summaryLookup,
    ...
    test('second read after recovery uses persisted summary, no re-recovery',
        () async {
    ...
    test('recovery does not trigger when summary already exists', () async {
      ...
      BaselineSelectionAnalyticsService.disableBridgeOnly();
      LearnBenchmarkContextService.testGetRestaurantId = () async => 'r1';
      LearnBenchmarkContextService.testGetProfile =
          (rid) async => testProfile;
      LearnBenchmarkContextService.testGetCycle = (rid) async => testCycle;
  ...
  group('O — profile-without-cycle recovery (8d)', () {
    ...
    test('profile + missing cycle + anchor recovers cycle and returns canonical context',
        () async {
      LearnBenchmarkContextService.disableBridgeOnly();
      BaselineSelectionAnalyticsService.disableBridgeOnly();
      ...
      LearnBenchmarkContextService.testGetAnchorDate =
          (rid) async => '2026-04-12';
      LearnBenchmarkContextService.testRecoverCycle =
          (rid, anchor) async => recoveredCycle;
      LearnBenchmarkContextService.testGetSummary =
          (cid) async => recoveredSummary;

      final ctx = await LearnBenchmarkContextService.instance.resolve();

      // Verify profile was re-read after recovery
      expect(profileReadCount, 2,
    ...
    test('recovery path no longer uses plain compatibility fallback',
        () async {
      LearnBenchmarkContextService.disableBridgeOnly();
      BaselineSelectionAnalyticsService.disableBridgeOnly();
      ...
      expect(recoverCycleCalled, isTrue,
          reason: 'missing cycle should trigger recovery, not bridge fallback');
    });

    test('recovered path repairs missing summary through 8c1 backfill',
        () async {
      LearnBenchmarkContextService.disableBridgeOnly();
      BaselineSelectionAnalyticsService.disableBridgeOnly();
    ...
    test('no anchor date for profile-without-cycle throws StateError',
        () async {
      LearnBenchmarkContextService.disableBridgeOnly();
