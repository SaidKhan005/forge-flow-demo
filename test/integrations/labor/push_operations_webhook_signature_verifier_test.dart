// Phase 8.gap-1 — Push Operations webhook signature verifier tests.
//
...
void main() {
  const verifier = PushOperationsWebhookSignatureVerifier();
  const signingSecret = 'push-operations-signing-secret-001';
  ...
  group('PushOperationsWebhookSignatureVerifier', () {
    test('vendorId mirrors the adapter constant', () {
      expect(verifier.vendorId, pushOperationsVendorId);
    });

    test('valid HMAC-SHA256 base64 signature returns true', () {
      final rawBody = Uint8List.fromList(
        utf8.encode('{"event":"shift.updated","id":"sh-001"}'),
      );
      final result = verifier.verify(
        rawBody: rawBody,
        headers: <String, String>{
          kPushOperationsSignatureHeader: signBase64(rawBody),
        },
    ...
    test('valid signature with epoch-seconds timestamp populates timestamp',
      ...
      final result = verifier.verify(
        ...
        headers: <String, String>{
          kPushOperationsSignatureHeader: signBase64(rawBody),
          kPushOperationsTimestampHeader: epochSeconds.toString(),
        },
        signingSecret: signingSecret,
        now: nowFixed,
      );
    ...
    test('invalid signature (wrong digest, same width) returns false', () {
      ...
      final result = verifier.verify(
        rawBody: rawBody,
        headers: <String, String>{
          kPushOperationsSignatureHeader:
    ...
    test('missing signature header returns false', () {
      ...
      expect(
        ...
        contains('missing X-Push-Signature header'),
      );
    });
    ...
    test('tampered body (one byte flip) breaks the signature', () {
      ...
      final result = verifier.verify(
        rawBody: tampered,
        headers: <String, String>{
          kPushOperationsSignatureHeader: providedSignature,
        },
        signingSecret: signingSecret,
        now: nowFixed,
      );
    ...
    test('empty body with valid signature is accepted', () {
      ...
      final result = verifier.verify(
        rawBody: empty,
        headers: <String, String>{
          kPushOperationsSignatureHeader: signBase64(empty),
        },
        signingSecret: signingSecret,
        now: nowFixed,
      );
    ...
    test('non-base64 signature returns false with parse-error reason', () {
      ...
      final result = verifier.verify(
        rawBody: rawBody,
        headers: const <String, String>{
          kPushOperationsSignatureHeader: 'not\$base64!!',
        },
        signingSecret: signingSecret,
        now: nowFixed,
      );
    ...
    test('header lookup is case-insensitive', () {
      ...
      final result = verifier.verify(
        rawBody: rawBody,
        headers: <String, String>{
          'X-Push-Signature': signBase64(rawBody),
        },
        signingSecret: signingSecret,
        now: nowFixed,
      );
