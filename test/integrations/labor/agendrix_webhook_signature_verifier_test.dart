// Phase 8.gap-1 — Agendrix webhook signature verifier tests.
//
...
void main() {
  const verifier = AgendrixWebhookSignatureVerifier();
  const signingSecret = 'agendrix-signing-secret-001';
  ...
  group('AgendrixWebhookSignatureVerifier', () {
    test('vendorId mirrors the adapter constant', () {
      expect(verifier.vendorId, agendrixVendorId);
    });

    test('valid HMAC-SHA256 base64 signature returns true', () {
      final rawBody = Uint8List.fromList(
        utf8.encode('{"event":"time_entry.updated","id":"te-001"}'),
      );
      final result = verifier.verify(
        rawBody: rawBody,
        headers: <String, String>{
          kAgendrixSignatureHeader: signBase64(rawBody),
        },
    ...
    test('valid signature with epoch-seconds timestamp populates timestamp',
      ...
      final result = verifier.verify(
        ...
        headers: <String, String>{
          kAgendrixSignatureHeader: signBase64(rawBody),
          kAgendrixTimestampHeader: epochSeconds.toString(),
        },
        signingSecret: signingSecret,
        now: nowFixed,
      );
    ...
    test('invalid signature (wrong digest, same width) returns false', () {
      ...
      final result = verifier.verify(
        rawBody: rawBody,
        headers: <String, String>{
          kAgendrixSignatureHeader: base64.encode(List<int>.filled(32, 0xAA)),
        },
        signingSecret: signingSecret,
        now: nowFixed,
      );
    ...
    test('missing signature header returns false', () {
      ...
      expect(
        ...
        contains('missing X-Agendrix-Signature header'),
      );
    });
    ...
    test('tampered body (one byte flip) breaks the signature', () {
      ...
      final result = verifier.verify(
        rawBody: tampered,
        headers: <String, String>{
          kAgendrixSignatureHeader: providedSignature,
        },
        signingSecret: signingSecret,
        now: nowFixed,
      );
    ...
    test('empty body with valid signature is accepted', () {
      ...
      final result = verifier.verify(
        rawBody: empty,
        headers: <String, String>{
          kAgendrixSignatureHeader: signBase64(empty),
        },
        signingSecret: signingSecret,
        now: nowFixed,
      );
    ...
    test('non-base64 signature returns false with parse-error reason', () {
      ...
      final result = verifier.verify(
        rawBody: rawBody,
        headers: const <String, String>{
          kAgendrixSignatureHeader: 'not\$base64!!',
        },
        signingSecret: signingSecret,
        now: nowFixed,
      );
    ...
    test('header lookup is case-insensitive', () {
      ...
      final result = verifier.verify(
        rawBody: rawBody,
        headers: <String, String>{
          'X-Agendrix-Signature': signBase64(rawBody),
        },
        signingSecret: signingSecret,
        now: nowFixed,
      );
