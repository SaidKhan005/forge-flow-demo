...
void main() {
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          contains('update provider_credentials'),
        );
        ...
        expect(tx.parameters[insertIdx]['masked_value'], equals('sk-a***1234'));
        expect(
  ...
  group('Contract 1 — per-tenant Future-lock under N parallel refreshes', () {
    test('5 parallel refresh attempts for the same (operator, location, '
      ...
      Future<TokenRefreshResult> doRefresh(
      ...
      expect(
        results,
        everyElement(equals('fresh-token')),
        reason:
    ...
    test('5 parallel refreshes against DIFFERENT (operator, location, '
        'vendor) tuples fire 5 distinct vendor RTTs (no cross-tenant '
        'lock contention)', () async {
  ...
  group('Contract 2 — atomic ciphertext rotation between vendor RTT and DB '
      'commit', () {
    test('concurrent reader during in-flight refresh observes a consistent '
        'ciphertext (never a half-rotated row)', () async {
      ...
      expect(
        probeReadings.single.isConsistent,
        isTrue,
        reason:
    ...
    test('a refresh that throws AFTER vendor RTT but BEFORE commit leaves the '
        'OLD ciphertext intact (rollback contract)', () async {
      final store = _AtomicCredentialStore()
        ..seed(
      ...
      expect(
        after.rotationCount,
        equals(0),
        reason: 'rotation_count must not advance for a failed refresh',
      );
    });

    test('after commit, all subsequent reads see the new ciphertext '
        '(post-rotation read-after-write)', () async {
      final store = _AtomicCredentialStore()
        ..seed(
  ...
  group('Contract 3 — oauth_refresh_advisory_lock migration semantics under N '
      'parallel refreshes', () {
    test('two cluster pods racing the same (operator_id, vendor_id): first '
        'acquires the lock and runs; second sees pg_try_advisory_xact_lock '
        'return false and SKIPS without firing a vendor RTT', () async {
      ...
      final podAResult = await OAuthRefreshCronRunner(
        gateway: podA,
        refreshers: <String, VendorOAuthRefresher>{'toast': podARefresher},
      ).runOnce();
      ...
      final podBResult = await OAuthRefreshCronRunner(
        gateway: podB,
        refreshers: <String, VendorOAuthRefresher>{'toast': podBRefresher},
      ).runOnce();

      expect(
        podAResult.refreshSuccesses,
        equals(1),
        reason: 'Pod A acquired the advisory lock and ran the refresh',
      );
      expect(podARefresher.refreshCalls, equals(1));
      expect(
        podBResult.refreshSuccesses,
        equals(0),
        reason:
      ...
      expect(lockState.acquisitionAttempts.last.key, equals('$_opA|toast'));
      expect(lockState.acquisitionAttempts.first.granted, isTrue);
      expect(lockState.acquisitionAttempts.last.granted, isFalse);
    });

    test('after Pod A commits, Pod B re-running its tick sees the row '
        ...
        'enter the runner loop at all', () async {
  ...
  group('Contract 4 — consecutive_refresh_failures counter contract', () {
    test('a refresh that fails increments consecutive_refresh_failures by 1; '
        'a subsequent successful refresh resets the counter to 0', () async {
      ...
      var result = await worker.runWorkerTick(
        ...
        refreshClosures: <String, worker.RefreshClosure>{
          'toast': (_) async => const TokenRefreshResult(accessToken: 'unused'),
        },
        maxRowsPerTick: 10,
      ...
      result = await worker.runWorkerTick(
        ...
        broker: broker,
        refreshClosures: <String, worker.RefreshClosure>{
          'toast': (_) async => const TokenRefreshResult(accessToken: 'fresh'),
        },
        maxRowsPerTick: 10,
        maxConsecutiveFailures: 3,
        horizon: const Duration(minutes: 5),
    ...
    test('counter reaches kRefreshFailureAutoDisableThreshold (3) — connection '
        'flips to error via gateway.autoDisableConnection', () async {
      final gateway = _CounterAwareWorkerGateway(
        credentials: <_CredentialKey, _CredentialState>{
          _CredentialKey(
            operatorId: _opA,
      ...
      expect(gateway.autoDisableInvocations.single.vendorId, equals('toast'));
    });

    test('no-closure vendors run through the worker tick: '
        'log-and-skip path does NOT call recordRefreshFailure (counter '
        'stays at 0 per worker registry docs)', () async {
      // Drive each registered no-closure vendor through one tick.
      ...
      // Sanity: pin the worker's current no-closure registry.
      expect(noClosureVendors, hasLength(4));
      expect(
        noClosureVendors.toSet(),
        equals(<String>{'agendrix', 'humanity', 'push_operations', 'tock'}),
        reason:
            'the no-closure set must match the vendors currently '
      ...
      expect(
        gateway.recordRefreshFailureInvocations,
        isEmpty,
        reason:
      ...
      // delegation surface).
      for (final vendorId in noClosureVendors) {
        final reason = worker.kVendorsWithoutRefreshClosureReason[vendorId]!;
        expect(
          out.lines.any(
            (line) => line.contains(vendorId) && line.contains(reason),
          ),
          isTrue,
          reason: '$vendorId log line must include reason "$reason"',
        );
      }
      await out.close();
    });
...
class _BrokerPool implements PostgresPool {
  _BrokerPool({this.rowsByTenant = const <String, Map<String, Object?>>{}});
...
class _TupleRowState {
  _TupleRowState({required this.ciphertext, required this.expiresAt});
  String ciphertext;
  DateTime expiresAt;
  DateTime? rotatedAt;
  int rotationCount = 0;
...
class _AtomicCredentialStore {
  ...
  String _key(_TupleKey t) => '${t.operatorId}|${t.locationId}|${t.vendorId}';
...
VendorCredentialRefreshRow _refreshRow({
  ...
  return VendorCredentialRefreshRow(
    ...
    tokenExpiresAt: DateTime.now().toUtc().add(const Duration(minutes: 30)),
    consecutiveFailures: 0,
  );
}
...
class _CountingRefresher implements VendorOAuthRefresher {
  ...
  Future<VendorRefreshOutcome> refresh({
    ...
    return VendorRefreshOutcome.success(
      ...
      newExpiresAt: DateTime.now().toUtc().add(const Duration(hours: 1)),
    );
  }
}
...
class _CredentialKey {
  ...
  @override
  int get hashCode =>
      Object.hash(operatorId, locationId, vendorId, credentialId);
}
...
class _CounterAwareWorkerGateway implements worker.OAuthRefreshWorkerGateway {
  ...
  Future<void> recordRefreshSuccess({
    ...
    credentials[key]?.consecutiveFailures = 0;
  }
