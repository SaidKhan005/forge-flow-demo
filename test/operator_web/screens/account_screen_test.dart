// Phase 11W.7 — Account screen widget tests.
//
// Covers the four sections (Profile, MFA, Password, T&Cs), the
// desktop (1024×768) + tablet (768×1024) breakpoints (including the
// Profile two-column → single-column flip), the role-based
// permission gate (`operator_owner` / `operator_admin` → full
// surface; `location_manager` → read-only with disabled CTAs), and
...
void main() {
  ...
  group('AccountScreen MFA section', () {
  ...
  group('AccountScreen Password section', () {
  ...
  group('AccountScreen T&Cs section', () {
  ...
  group('AccountScreen permission gate', () {
    ...
    testWidgets('demo location_manager factory lands on the read-only branch',
      ...
      await tester.pumpWidget(
        wrap(AccountScreen(session: completed.session)),
      );
