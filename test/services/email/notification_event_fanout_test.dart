...
//     channel.
//   * Admin-only events admit operator owners only; manager-only events
//     admit the current v2 manager roles only.
//   * Retired v1 roles (`operator_admin`, `operator_manager`) do not
//     satisfy notification gates.
...
void main() {
  group('NotificationEventFanout.fanOut', () {
    ...
    test('catalog defaults apply when no preference row exists', () async {
      ...
      expect(emails.calls.single.recipientEmail, 'owner@acme.test');
      // Salutation derived from email local-part.
      expect(emails.calls.single.templateData['recipientName'], 'owner');
    });

    test(
      'enabled=false preference opts user out of a default channel',
      () async {
    ...
    test('enabled=true admits a channel the catalog default omits', () async {
      ...
      final inboxCall = pushes.calls.firstWhere((p) => p.routesToInbox);
    ...
    test('admin-only events admit operator_owner only', () async {
      ...
      final fanout = NotificationEventFanout(
        ...
        userDirectory: ({required String operatorId}) async => <FanoutUser>[
          _ownerUser(),
          // Retired phantom admin - not admitted.
          const FanoutUser(
            userId: 'user-2',
            locationId: 'loc-1',
            roles: <String>{'operator_admin'},
            email: 'phantom-admin@acme.test',
          ),
          // Current v2 manager - not admitted to owner-only events.
          const FanoutUser(
            userId: 'user-3',
            locationId: 'loc-1',
    ...
    test('manager-only events admit current v2 manager roles only', () async {
      ...
      final fanout = NotificationEventFanout(
        ...
        userDirectory: ({required String operatorId}) async => <FanoutUser>[
          ...
          // Retired v1 roles are not admitted.
          const FanoutUser(
            userId: 'user-5',
            locationId: 'loc-1',
            roles: <String>{'operator_admin'},
          ...
          const FanoutUser(
            ...
            email: 'retired-manager@acme.test',
          ),
      ...
      expect(outcome.usersGated, 3);
      // notif.shift.stale catalog default = {push} only (no email).
      expect(outcome.pushDispatched, 4);
      expect(outcome.emailDispatched, 0);
      expect(pushes.calls.map((payload) => payload.userId).toList(), <String>[
        'user-1',
        'user-2',
        'user-3',
        'user-4',
    ...
    test('idempotent dedupe key: same envelope -> same key', () async {
      ...
      expect(emails.calls[0].idempotencyKey, emails.calls[1].idempotencyKey);
      expect(
    ...
    test('per-channel payload shape', () async {
      ...
      expect(email.recipientEmail, 'owner@acme.test');
      expect(email.templateId, 'backfill_complete');
      expect(email.templateData['vendorId'], 'toast');
      expect(email.templateData['recipientName'], 'owner');
    });
...
FanoutUser _ownerUser() => const FanoutUser(
  userId: 'user-1',
  locationId: 'loc-1',
  roles: <String>{'operator_owner'},
  email: 'owner@acme.test',
...
class _RecordingEmailSeam {
  ...
  Future<void> dispatch(FanoutEmailPayload payload) async {
    if (failOnTemplateId != null && payload.templateId == failOnTemplateId) {
