...
void main() {
    ...
    test(
      ...
      () async {
        ...
        final selectSql = txA.executedSql.firstWhere(
          (s) =>
              s.contains('from effective_forge_flow_polling_tier_assignment_v'),
        );
  ...
  group('ForgeFlowPollingTierRepository — assignTier transition (item F)', () {
    test('assignTier closes the prior current row with '
        'effective_until = now(), then INSERTs the new row, all within '
        'a single transaction', () async {
      final pool = _TierPool(
  ...
  group('ForgeFlowPollingTierRepository — assignment history (item G)', () {
    test('listAssignmentHistory orders by effective_at desc; current row '
        '(effective_until is null) appears first', () async {
      ...
      final pool = _TierPool(historyRows: <PostgresRow>[current, older]);
      final repo = ForgeFlowPollingTierRepository(
        TenantTransactionWrapper(pool),
      );
      ...
      expect(
        ...
        reason:
            'history SELECT must NOT filter to current — it lists '
  ...
  group('ForgeFlowPollingTierRepository — margin summary (item I)', () {
    test('summarizeMargin aggregates currently-effective rows; '
        'totalMonthlyMarginCents = price - cost; marginFraction returns '
        'the right ratio', () async {
      final pool = _TierPool(
      ...
      final tx = pool.transactions.single;
      final selectSql = tx.executedSql.firstWhere(
        (s) => s.contains('from effective_forge_flow_polling_tier_assignment_v'),
      );
      expect(
        selectSql,
        contains('and effective_until is null'),
    ...
    test('summarizeMargin with tier_key filter narrows the SELECT to a '
        'single tier (used by the per-tier breakdown card)', () async {
      final pool = _TierPool(
        marginRow: <String, Object?>{
          'assignment_count': 1,
    ...
    test('summarizeMargin with zero rows returns a sensible zero summary '
        '(marginFraction null because no revenue)', () async {
      final pool = _TierPool(
        marginRow: <String, Object?>{
          'assignment_count': 0,
          'total_price_cents': 0,
...
class _TierTransaction extends PostgresTransaction {
  ...
  Future<List<PostgresRow>> query(
    ...
    if (sql.contains('from effective_forge_flow_polling_tier_assignment_v')) {
