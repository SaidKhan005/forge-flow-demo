...
void main() {
  group('HealthEnvelope.fromJson', () {
    test('parses dependencies, surfaces, metrics from a green envelope', () {
      ...
      expect(
        envelope.dependencies.map((d) => d.name).toSet(),
        <String>{'postgres', 'age', 'pgvector'},
      );
  ...
  group('InMemoryHealthAdminGateway', () {
    test('returns the seeded envelope and respects setEnvelope updates',
        () async {
    ...
    test('setEnvelope errorOnFetch surfaces the configured exception',
        () async {
      final gateway = InMemoryHealthAdminGateway(
        envelope: _greenEnvelopeJson(),
      );
