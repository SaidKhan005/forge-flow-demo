...
void main() {
  group('LocationsRepository.listForOperator (admin tenant-bounded sweep)', () {
    test(
      'uses withSystem with the canonical bypass marker; SQL filters '
      'operator_id::uuid and orders by created_at so the admin sees a '
      'stable per-operator listing even on the BYPASSRLS path',
      () async {
        ...
        expect(
          auditCfg['value'],
          equals('system:admin.locations.list_for_op'),
        );
        ...
        expect(
          ...
          isEmpty,
          reason: 'withSystem must not set tenant GUCs — that would shadow '
    ...
    test('rejects a blank adminReason at the wrapper boundary', () async {
      ...
      expect(
        pool.transactions,
        isEmpty,
        reason: 'wrapper validates the audit marker before opening a tx — '
            'a blank reason cannot mint an unattributable BYPASSRLS path',
      );
    });

    test('returns an empty list when the operator has no locations',
        () async {
  ...
  group('LocationsRepository.listAllLocations (cross-operator admin sweep)',
      () {
    test(
  ...
  group('LocationsRepository.insertLocation', () {
    test(
      'business_timezone IANA string round-trips parametrically (no '
      'concatenation, no normalization on the way down)',
      () async {
        final pool = _LocationsPool(
          insertedRows: <PostgresRow>[
            _locationRow(timezone: 'Pacific/Auckland'),
          ],
        );
        final repo = LocationsRepository(TenantTransactionWrapper(pool));
    ...
    test(
      'business_day_rollover_hour boundary: 0 (midnight rollover) and '
      '23 (last legal hour) round-trip unchanged on INSERT bind',
      () async {
        // 0 — midnight rollover.
        final poolZero = _LocationsPool(
          insertedRows: <PostgresRow>[
            _locationRow(businessDayRolloverHour: 0),
          ],
        );
        final repoZero = LocationsRepository(
          TenantTransactionWrapper(poolZero),
        );
        ...
        final paramsZero = poolZero.transactions.single.parameters
            .firstWhere((p) => p['name'] == 'Midnight');
        ...
        final pool23 = _LocationsPool(
          insertedRows: <PostgresRow>[
            _locationRow(businessDayRolloverHour: 23),
          ],
        );
        ...
        final params23 = pool23.transactions.single.parameters
            .firstWhere((p) => p['name'] == 'LateNight');
        expect(params23['business_day_rollover_hour'], equals(23));
      },
    );

    test(
      'binds operator_id::uuid + name + address + timezone + '
      'business_day_rollover_hour exactly once each in the INSERT',
      () async {
        final pool = _LocationsPool(
    ...
    test(
      'throws StateError when the INSERT RETURNING is empty — RLS '
  ...
  group('LocationsRepository.updateLocation', () {
    test(
      'coalesce guards every editable column — omitted fields are '
      ...
      () async {
        ...
        expect(
          updateSql,
          contains('timezone = coalesce(@timezone, timezone)'),
        );
        expect(
          updateSql,
          contains(
    ...
    test(
      'business_timezone update path — passing a new IANA value binds '
      'parametrically; coalesce switches to the new value',
      () async {
        final pool = _LocationsPool(
          updatedRows: <PostgresRow>[
            _locationRow(timezone: 'Europe/Berlin'),
          ],
        );
        final repo = LocationsRepository(TenantTransactionWrapper(pool));
    ...
    test(
      'business_day_rollover_hour update path — both endpoints (0 and '
      '23) bind unchanged through the coalesce wrapper',
      () async {
        final pool = _LocationsPool(
          updatedRows: <PostgresRow>[
            _locationRow(businessDayRolloverHour: 0),
    ...
    test(
      'returns null when UPDATE … RETURNING yields no rows (location '
      'does not exist — proxy translates to a 404)',
      () async {
  ...
  group('LocationsRepository.deleteLocation (cross-operator isolation)', () {
    ...
    test(
      'returns 0 when the location does not belong to the operator '
      '(proxy translates that into a 404; the cross-operator predicate '
      'is what makes this safe under BYPASSRLS)',
      () async {
