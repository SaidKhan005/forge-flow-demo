...
void main() {
  ...
  group('Group 1 — cutoff + IANA-zone round-trip', () {
    test('createProfile + loadProfileById round-trips the rollover '
      ...
      await withTestPostgres((pool, wrapper) async {
        ...
        final candidates = await repo.listCandidateProfilesForLocation(
          operatorId: _opA,
          locationId: _locA1,
    ...
    test('rollover-cutoff boundary cases — 00:00 (midnight) and 23:00 '
        '(one-hour-before-midnight) both persist; both honour the '
        'quarter-hour check constraint', () async {
      await withTestPostgres((pool, wrapper) async {
        await seedOperator(pool, operatorId: _opA, locationId: _locA1);
        final repo = BusinessTimingProfilesRepository(wrapper);
  ...
  group('Group 2 — hierarchy-scoped resolution', () {
    test('business + location overrides — operator-scope profile is '
        'inherited; location-scope profile wins when present', () async {
      await withTestPostgres((pool, wrapper) async {
        await seedOperator(pool, operatorId: _opA, locationId: _locA1);
        final repo = BusinessTimingProfilesRepository(wrapper);
        ...
        final inheritedCandidates = await repo.listCandidateProfilesForLocation(
          operatorId: _opA,
          locationId: _locA1,
          businessDate: '2026-05-09',
          userId: _actorA,
        ...
        final overriddenCandidates = await repo
    ...
    test('"inherited from" metadata identifies the source scope — every '
        'candidate row exposes scope_type/scope_id so the operator-web '
        'editor can render the resolution provenance', () async {
      await withTestPostgres((pool, wrapper) async {
        await seedOperator(pool, operatorId: _opA, locationId: _locA1);
        final repo = BusinessTimingProfilesRepository(wrapper);
        await repo.createProfileAsSystem(
  ...
  group('Group 3 — business_date derivation honours the cutoff', () {
    ...
    test('cutoff 00:00 — every wall-clock instant on a calendar date '
        'maps to that calendar date as business_date', () async {
      await withTestPostgres((pool, wrapper) async {
        await seedOperator(pool, operatorId: _opA, locationId: _locA1);
        final repo = BusinessTimingProfilesRepository(wrapper);
        await repo.createProfileAsSystem(
    ...
    test('DST spring-forward — IANA-tz business-date converter pinned '
        ...
        'after the skipped wall-clock window.', () async {
      await withTestPostgres((pool, wrapper) async {
        await seedOperator(pool, operatorId: _opA, locationId: _locA1);
    ...
    test('DST fall-back — IANA-tz business-date converter against '
        ...
        'projected through the IANA database.', () async {
      await withTestPostgres((pool, wrapper) async {
        await seedOperator(pool, operatorId: _opA, locationId: _locA1);
        final converter = IanaTimezoneConverter();
  ...
  group('Group 4 — operator-scoped read isolation', () {
    test('tenant A creates a profile; tenant B running through '
        'listProfilesForOperator with operator A\'s id sees nothing '
        '(RLS denies under tenant B SET LOCAL)', () async {
      await withTestPostgres((pool, wrapper) async {
        await seedOperator(pool, operatorId: _opA, locationId: _locA1);
        ...
        expect(aRows.single.businessDayStartLocalTime, equals('04:00'));
        ...
        expect(bRows.single.businessDayStartLocalTime, equals('06:00'));
      });
    });

    test('system-scope listing (`set local role forge_admin`) sees both '
        'tenants — escape hatch is preserved for support flows', () async {
      await withTestPostgres((pool, wrapper) async {
        await seedOperator(pool, operatorId: _opA, locationId: _locA1);
        await seedOperator(pool, operatorId: _opB, locationId: _locB1);
        final repo = BusinessTimingProfilesRepository(wrapper);
  ...
  group('Group 5 — `op.rollover_hour does not exist` regression', () {
    test('locations.business_day_rollover_hour column exists with int '
        'type and 0..23 check constraint — fresh schema must support '
        'the production business_date derivation', () async {
      await withTestPostgres((pool, wrapper) async {
        ...
        try {
          ...
          expect(
            rows.single['column_name'],
            equals('business_day_rollover_hour'),
          );
    ...
    test('production read path runs end-to-end against a fresh schema '
        ...
        'clear postgres error rather than a sanitized 5xx', () async {
      await withTestPostgres((pool, wrapper) async {
        await seedOperator(pool, operatorId: _opA, locationId: _locA1);
        final repo = BusinessTimingProfilesRepository(wrapper);
        await repo.createProfileAsSystem(
        ...
        expect(candidates.single.businessDayStartLocalTime, equals('04:00'));
        expect(
