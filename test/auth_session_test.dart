...
void main() {
  ...
  group('MfaChallengeScreen widget', () {
    ...
    testWidgets('enters a TOTP code and submits the challenge', (tester) async {
      final service = _FakeAuthLoginService();
      final notifier = AuthSessionNotifier(
      ...
      notifier.debugSetState(
        const AuthSessionMfaChallenge(
          ...
          factorIds: <String>['totp-factor'],
        ),
      );
      service.totpResult = AuthLoginSuccess(
        AuthSession(
          userId: 'u',
          operatorId: 'o',
          locationId: 'l',
      ...
      expect(service.lastTotpFactorId, equals('totp-factor'));
      expect(service.lastTotpCode, equals('123456'));
      expect(notifier.state, isA<AuthSessionAuthenticated>());
    });
  });

  group('LoginScreen widget', () {
    Widget wrap(AuthSessionNotifier notifier, {LoginScreen? screen}) {
      return MaterialApp(
        home: ChangeNotifierProvider<AuthSessionNotifier>.value(
          value: notifier,
          child: screen ?? const LoginScreen(),
        ),
      );
    }
    ...
    testWidgets('demo operator button fills credentials and submits', (
      tester,
    ) async {
      final session = AuthSession(
        userId: 'u',
      ...
      await tester.pumpWidget(
        wrap(
          ...
          screen: const LoginScreen(
            showDemoOperatorSignIn: true,
            demoOperatorEmail: 'demo.operator@example.test',
            demoOperatorPassword: 'demo-password',
          ),
        ),
      );
      await tester.pumpAndSettle();
      ...
      expect(service.signInCalls, equals(1));
      expect(service.lastEmail, equals('demo.operator@example.test'));
      expect(service.lastPassword, equals('demo-password'));
      expect(notifier.state, isA<AuthSessionAuthenticated>());
    });
...
class _FakeAuthLoginService implements AuthLoginService {
  ...
  Future<AuthLoginResult> completeTotpChallenge({
    ...
    totpCalls += 1;
    lastTotpFactorId = factorId;
    lastTotpCode = oneTimeCode;
    return totpResult ??
        const AuthLoginFailure(
          code: 'unconfigured',
          message: 'fake service did not specify a result',
