...
class _AccountScreenState extends State<AccountScreen> {
  ...
  /// Returns the scope's own [scopeValue], else the [inherited] value,
  /// else [fallback]. Kept tiny so the seed method stays branch-free.
  T _firstNonNull<T>(T? scopeValue, T? inherited, T fallback) =>
      scopeValue ?? inherited ?? fallback;

  void _seedAccountFormFromEnvelope(LocationAccountOverridesEnvelope envelope) {
    final o = envelope.override;
    final d = envelope.businessDefault;
    _currencyCode = _firstNonNull(
      o.currencyCode,
      d.currencyCode,
      _currencyCode,
    );
    _localeTag = _firstNonNull(o.localeCode, d.localeCode, _localeTag);
    _rolloverHour = _firstNonNull(
      o.businessDayRolloverHour,
      d.businessDayRolloverHour,
      _rolloverHour,
    );
    _contactEmail.text = _firstNonNull(o.contactEmail, d.contactEmail, '');
    _contactPhone.text = _firstNonNull(o.contactPhone, d.contactPhone, '');
    _savedTimezoneValue = _firstNonNull(
      o.ianaTimezone,
      d.ianaTimezone,
      _savedTimezoneValue,
    );
  ...
  /// True when the Reset-to-inherited button should be tappable: the
  /// actor can edit, a gateway is wired, no save is in flight, the scope
  /// is editable, and there is at least one local override to clear.
  bool _resetToInheritedEnabled(bool lowerScopeEditable) =>
      widget.canEdit &&
      _hasGateway &&
      !_submitting &&
      lowerScopeEditable &&
  ...
  /// Confirmation prompt shown before clearing a scope's overrides.
  Future<bool> _confirmResetToInherited() async {
  ...
  /// Sends the all-clear patch for the current below-business scope and
  /// returns the refreshed envelope, or null when the scope/gateway
  /// preconditions are not met. Business scope has no parent to inherit
  /// from, so it is excluded.
  Future<LocationAccountOverridesEnvelope?> _clearScopeOverrides() async {
    final gateway = widget.gateway;
    final scope = widget.accountOverrideScope;
    if (gateway == null || !widget.canEdit || scope == null) return null;
    if (!widget.scopeBelowBusiness) return null;
    final scopeGateway = _scopeOverridesGateway;
    final locationId = widget.locationIdForOverride;
    if (scopeGateway == null && locationId == null) return null;
  ...
  /// Clears every account override at the current scope so the values
  /// fall back to the ones inherited from the higher scope.
  Future<void> _handleResetToInherited() async {
    if (!await _confirmResetToInherited() || !mounted) return;
    setState(() {
      _submitting = true;
      _errorMessage = null;
    ...
    try {
      final envelope = await _clearScopeOverrides();
      if (!mounted) return;
      if (envelope == null) {
                ...
                if (widget.scopeBelowBusiness)
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      ...
                      onPressed: _resetToInheritedEnabled(lowerScopeEditable)
