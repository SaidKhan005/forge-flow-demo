// Phase 8.gap-1 — Push Operations webhook signature verifier
...
// against the assumed Push Operations webhook signing scheme. The
// Push Operations API documents no webhook delivery surface —
// `push_operations_labor_adapter.dart` declares
...
// verifier" warning at boot and to give a future Push Operations
// webhook release a documented landing surface that mirrors every
// other vendor.
//
...
//   * Header          : `X-Push-Signature`. Lookup is
//                       case-insensitive; the framework lower-cases
//                       header keys before dispatch.
//   * Timestamp header: `X-Push-Timestamp` (Unix epoch seconds).
...
/// dispatch.
const String kPushOperationsSignatureHeader = 'x-push-signature';
...
const String kPushOperationsTimestampHeader = 'x-push-timestamp';

/// Push Operations documented webhook signature verifier. Stateless;
/// `VendorWebhookSignatureVerifier.verify(...)` does not mutate the
/// instance. One default-constructed verifier per process is fine.
class PushOperationsWebhookSignatureVerifier
  ...
  WebhookSignatureVerification verify({
    ...
    final providedSignature =
        _readHeader(headers, kPushOperationsSignatureHeader);
    if (providedSignature == null || providedSignature.isEmpty) {
      return const WebhookSignatureVerification(
        valid: false,
        failureReason: 'missing X-Push-Signature header',
      );
    }
    ...
    try {
      ...
      return WebhookSignatureVerification(
        valid: false,
        failureReason:
            'X-Push-Signature is not valid base64: ${error.message}',
      );
    }
    ...
    if (!constantTimeBytesEquals(providedBytes, expected)) {
      return const WebhookSignatureVerification(
        valid: false,
        failureReason: 'HMAC mismatch on X-Push-Signature header',
      );
    }
  ...
  /// Push Operations' optional `X-Push-Timestamp` is Unix epoch seconds.
  ...
  DateTime? _readTimestamp(Map<String, String> headers) {
    final raw = _readHeader(headers, kPushOperationsTimestampHeader);
