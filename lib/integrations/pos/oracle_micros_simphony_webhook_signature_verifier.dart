...
// against the assumed Oracle MICROS Simphony webhook signing scheme.
// The Simphony Cloud API does not document a webhook delivery surface
// — `oracle_micros_simphony_pos_adapter.dart` declares
// `webhookSupport = pollOnly` and the framework router will not
// dispatch webhook traffic to this verifier under normal operation.
// The verifier exists to close the production binder's "missing
// verifier" warning at boot and to give a future Simphony webhook
// release (or partner-program private webhook offer) a documented
// landing surface that mirrors every other vendor.
//
// Documented algorithm (industry-standard Oracle partner convention,
// to be verified live in `8.gap-1.live.sandbox`):
//   * Signed payload  : raw HTTP body bytes (no JSON re-serialization).
//   * Algorithm       : HMAC-SHA256.
//   * Encoding        : base64 (standard, with padding).
//   * Header          : `X-Oracle-Signature`. Lookup is
//                       case-insensitive; the framework lower-cases
//                       header keys before dispatch.
//   * Timestamp header: `X-Oracle-Timestamp` (Unix epoch seconds).
...
/// dispatch.
const String kOracleMicrosSimphonySignatureHeader = 'x-oracle-signature';
...
const String kOracleMicrosSimphonyTimestampHeader = 'x-oracle-timestamp';

/// Oracle MICROS Simphony documented webhook signature verifier.
/// Stateless; `VendorWebhookSignatureVerifier.verify(...)` does not
/// mutate the instance. One default-constructed verifier per process
/// is fine.
class OracleMicrosSimphonyWebhookSignatureVerifier
  ...
  WebhookSignatureVerification verify({
    ...
    final providedSignature =
        _readHeader(headers, kOracleMicrosSimphonySignatureHeader);
    if (providedSignature == null || providedSignature.isEmpty) {
      return const WebhookSignatureVerification(
        valid: false,
        failureReason: 'missing X-Oracle-Signature header',
      );
    }
    ...
    try {
      ...
      return WebhookSignatureVerification(
        valid: false,
        failureReason:
            'X-Oracle-Signature is not valid base64: ${error.message}',
      );
    }
    ...
    if (!constantTimeBytesEquals(providedBytes, expected)) {
      return const WebhookSignatureVerification(
        valid: false,
        failureReason: 'HMAC mismatch on X-Oracle-Signature header',
      );
    }
  ...
  /// Oracle's optional `X-Oracle-Timestamp` is Unix epoch seconds.
  ...
  DateTime? _readTimestamp(Map<String, String> headers) {
    final raw = _readHeader(headers, kOracleMicrosSimphonyTimestampHeader);
