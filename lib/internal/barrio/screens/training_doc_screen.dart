...
class _TrainingHero extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // one-line title ellipsizes, and the title's text scaling is
    // CLAMPED at 1.45x. Clamp rationale (documented per the WCAG 1.4.4
    // "no loss of content or function" bar): the one-line cap is a
    // deliberate v2 decision keeping the card window height steady
    // mid-session; letting a 26px Playfair title scale to 2.0x would
    // ellipsize almost every section title into meaninglessness, while
    // at 1.45x the common titles still fit. The full title stays
    // available in the header semantics label and on the rail tile
    // labels, so no content is lost at any scale.
    return Semantics(
      header: true,
      label: 'Section ${sectionIndex + 1} of $sectionCount: ${chapter.title}',
