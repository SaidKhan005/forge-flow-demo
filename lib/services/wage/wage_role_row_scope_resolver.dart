...
//
// Schema anchor: db/migrations/202605161800_gap_b2_wage_role_rows_
// hierarchy_scope.sql adds scope_type / org_unit_id /
...
class WageRoleRowScopeResolver {
  ...
  WageRoleRowResolvedValue resolve({
    ...
    final operatorMatch = _latest(
      matching.where(
        (candidate) =>
            candidate.scopeType == WageRoleRowScopeType.operatorWide,
      ),
    );
  ...
  WageRoleRowScopeCandidate? _latest(
    ...
    for (final candidate in candidates) {
      if (best == null ||
          candidate.effectiveAt.isAfter(best.effectiveAt)) {
        best = candidate;
      }
    }
    return best;
