// Phase 8 / Wave B `8.spine-bridge.1.NR` — NCR Simphony POS
// Postgres-backed canonical sink.
//
// Spine reference:
//   * `docs/contracts/integration_spine_architecture_contract.md`
//     section "8.spine-bridge.1.NR" (binding) — same shape as `.1.OR`.
...
// What this lane does:
//
//   1. Implements both the bespoke [NcrFactSink]
//      (consumed by `NcrPosAdapter`) AND the unified
...
//      `_canonicalize` in `ncr_pos_adapter.dart`) to
...
//      false). The NCR vendor-shape check is preserved in the
...
//
//   7. Tolerates `covers = null` in the canonical-fact dict. NCR's
...
// `test/infrastructure/persistence/postgres/ncr_pos_postgres_sink_test.dart`
...
const String kNcrVendorId = 'ncr';

/// `connector_sync_watermark.resource` value the NCR sink writes
...
const String ncrWatermarkResource = 'pos.guest_checks';

/// Postgres-backed canonical sink for NCR Simphony POS.
///
/// Implements both the bespoke [NcrFactSink] (consumed by
/// the adapter at `lib/integrations/pos/ncr_pos_adapter.dart`)
...
class NcrPostgresSink extends OperatorScopedRepository
  ...
  Future<bool> _writeCoverFact({
    ...
    final inserted = await withTenant<bool>(ctx, (exec) async {
      ...
      if (closedAt == null) {
        // NCR `closedAt` is required for the business-date
    ...
    return withTenant<String>(ctx, (exec) async {
      ...
      if (rows.isEmpty) {
        throw StateError(
          ...
          'vendor=$kNcrVendorId) — connect must run '
          'before sink writes',
        );
      }
