// Phase 8 / Wave B `8.spine-bridge.1.AL` — Aloha POS
...
// What this lane does:
//
//   1. Implements both the bespoke [AlohaFactSink]
//      (consumed by `AlohaPosAdapter`) AND the unified
...
//      `_canonicalize` in `aloha_pos_adapter.dart`) to
...
// `test/infrastructure/persistence/postgres/aloha_pos_postgres_sink_test.dart`
...
const String kAlohaVendorId = 'aloha';

/// `connector_sync_watermark.resource` value the Aloha sink writes
...
const String alohaWatermarkResource = 'pos.guest_checks';
...
class AlohaPostgresSink extends OperatorScopedRepository
    ...
    await withTenant<void>(ctx, (exec) async {
      await exec.execute(
        ...
        parameters: <String, Object?>{
          ...
          'resource': alohaWatermarkResource,
  ...
  /// (the Aloha adapter) do not carry the id; the unified-interface
    ...
    return withTenant<String>(ctx, (exec) async {
      ...
      if (rows.isEmpty) {
        throw StateError(
          ...
          'vendor=$kAlohaVendorId) — connect must run '
          'before sink writes',
        );
      }
