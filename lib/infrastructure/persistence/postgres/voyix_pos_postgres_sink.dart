// Phase 8 / Wave B `8.spine-bridge.1.VX` — Voyix POS
// Postgres-backed canonical sink.
//
// Spine reference:
//   * `docs/contracts/integration_spine_architecture_contract.md`
//     section "8.spine-bridge.1.VX" (binding) — same shape as `.1.OR`.
...
// What this lane does:
//
//   1. Implements both the bespoke [VoyixFactSink]
//      (consumed by `VoyixPosAdapter`) AND the unified
...
//      `_canonicalize` in `voyix_pos_adapter.dart`) to
...
//      false). The Voyix vendor-shape check is preserved in the
//      `raw_payload` JSONB column for forensic re-derivation.
//
//   3. Computes `business_date` at write time from
//      `cover_facts.closed_at` (projected from the Voyix `closedAt`
...
//
//   7. Tolerates `covers = null` in the canonical-fact dict. Voyix's
...
// `test/infrastructure/persistence/postgres/voyix_pos_postgres_sink_test.dart`
...
/// Vendor identifier for Voyix POS systems.
const String kVoyixVendorId = 'voyix';

/// `connector_sync_watermark.resource` value the Voyix sink writes
...
const String voyixWatermarkResource = 'pos.guest_checks';

/// Postgres-backed canonical sink for Voyix POS.
///
/// Implements both the bespoke [VoyixFactSink] (consumed by
/// the adapter at `lib/integrations/pos/voyix_pos_adapter.dart`)
...
class VoyixPostgresSink extends OperatorScopedRepository
  ...
  // Per-Daypart V1 / Slice 7b option (b) (2026-05-15): Voyix no longer
  ...
  Future<bool> upsertCoverFact({
    ...
    // the raw vendor payload under the `raw_payload` key. Defensive
    // coerce: if the dispatcher omits the key the column stores `{}`
  ...
  Future<bool> _writeCoverFact({
    ...
    final inserted = await withTenant<bool>(ctx, (exec) async {
      ...
      if (closedAt == null) {
        // Voyix `closedAt` is required for the business-date
    ...
    await withTenant<void>(ctx, (exec) async {
      await exec.execute(
        ...
        parameters: <String, Object?>{
          ...
          'resource': voyixWatermarkResource,
  ...
  /// (operator, location, vendor) triple. When [explicitConnectionId] is
    ...
    return withTenant<String>(ctx, (exec) async {
      ...
      if (rows.isEmpty) {
        throw StateError(
          ...
          'vendor=$kVoyixVendorId) — connect must run '
          'before sink writes',
        );
      }
