...
class _MembersAdminScreenState extends State<MembersAdminScreen> {
  ...
  List<RoleAdminRow> get _teamMemberRoles {
    return _roles.where((role) => !role.isPlatformRole).toList(growable: false);
  }

  List<MemberRoleRef> get _inviteRoleOptions {
    return <MemberRoleRef>[
      for (final role in _roleFilterOptions)
        MemberRoleRef(roleKey: role.roleKey, label: role.label),
    ];
  }
