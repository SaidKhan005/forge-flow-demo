...
- **In-browser QA — operator web console.** Any session asked to run, pressure-test, or QA the operator web console MUST read `runbooks/operator_web_qa_runbook.md` before doing anything else. The build command, dhttpd serve config (`operator-web-static`, port 8185), polyfill boot-verify snippet, semantics-enable snippet, **canonical assertion runner at `web/_qa_runner.js` (Step 5 — paste into `preview_eval` and read the pass/fail summary)**, 11-route checklist, known baseline defects, and troubleshooting table are all there. No re-discovery.
- **In-browser QA — admin console.** Any session asked to run, pressure-test, or QA the admin console MUST read `runbooks/admin_console_browser_qa_runbook.md` before doing anything else. Same polyfill methodology; `--dart-define=ADMIN_SHARE_PREVIEW_AS_SUPER_ADMIN=true` build; dhttpd serve config (`admin-web-static`, port 8186); **canonical assertion runner at `web/_qa_runner_admin.js` (Step 5 — inject via `<script src="/_qa_runner_admin.js">` and read `window.__adminQaResult`; baseline 2026-05-26 is PASSED 124/124 covering 5 boot checks + 27 shell + identity assertions + 18 primary nav routes + 1 interaction + 1 RenderFlex-overflow guard + 2 passive regressions for the known share-preview wedges PRD-01 / PRD-02);** 18-route checklist covering every top-level admin nav row. No re-discovery.
