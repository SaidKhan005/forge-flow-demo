...
- **No per-request token cap on outbound LLM calls.** Repo-wide search for `MAX_TOKENS_PER_REQUEST`, `requestTokenCap`, etc. returns zero matches. The proxy's outbound LLM call sites (`tool/advisor_proxy/advisor_proxy.dart:8485-8700` where `tokenCount` and `UsageEstimate.requestTokens` flow) have no enforced cap. Spec called for one — none was wired. **Still open**, separate from the misdiagnosed `cost_telemetry_limit` bullet.
...
- **`cost_telemetry_limit` query param** — the audit flagged this as a bypassable token cap, but it's a row-count `LIMIT` on the admin observability dashboard's pagination, server-clamped to `[1, 100]`. No token cap to bypass. **Closed as audit misdiagnosis** (verified at `tool/advisor_proxy/advisor_proxy.dart:13822-13847` and `tool/advisor_proxy/proxy_bootstrap.dart:5670-5730`).
