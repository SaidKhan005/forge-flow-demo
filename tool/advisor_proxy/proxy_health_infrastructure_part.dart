// Forge & Flow advisor proxy — health infrastructure (part of advisor_proxy.dart).
//
// chore(advisor-proxy): pure size refactor. This part file holds the
// proxy health status, metric, and surface classes together with their
// reserved metric catalogs and dependency probe/registry infrastructure,
// mechanically lifted out of `advisor_proxy.dart` so the monolith stays
// under the `kAdvisorProxyMaxLines` bleed-stop ceiling enforced by
...
///
/// A1 Â§2.4 instrumentation items #3 and #4 â€” surface S2 (pool waiter
...
class ProxyHealthStatus {
  ...
  /// + 6 surfaces â€” used as a rollback path if the expanded envelope
  /// regresses.
...
String _rollUpSurfaceStatus(
  ...
  if (hasGreen) return 'green'; // some unknown but no red/yellow â†’ green wins.
  return 'unknown';
}
...
/// Full B42 producer catalog â€” 58 reserved metric slots covering every
...
proxyHealthReservedMetrics = <String, ProxyHealthMetric>{
  // â”€â”€ Tier 1 â€” foundation/auth/audit â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  'audit_chain_lag_seconds': ProxyHealthMetric(
    status: 'unknown',
    value: null,
    unit: 'seconds',
  ...
  // â”€â”€ Tier 2 â€” production hardening â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  'event_outbox_undelivered_count': ProxyHealthMetric(
    status: 'unknown',
    value: null,
    unit: 'count',
  ...
  // Phase 10a.4 â€” bridge-side post-pickup lag. Same Q22 thresholds as
  ...
  'graph_growth_projection_90d_edges': ProxyHealthMetric(
    ...
    unit: 'count',
    description:
        '90-day projected edge count (Decision 30: red â‰¥ 5M triggers '
  ...
  // â”€â”€ Tier 3 â€” ops observability â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // Slice A11.1.b â€” surface the per-instance SessionRecordIncompleteGauge
  ...
  // wave_completion_deep_audit_2026_05_13.md finding #2 + R3 Â§2 stretch
    ...
    <String, ProxyHealthSurface>{
      ...
      'event_outbox': ProxyHealthSurface(
        ...
        metrics: <String>[
          ...
          // Phase 10a.4 â€” bridge-side post-pickup lag (sister metric to
      ...
      'infra': ProxyHealthSurface(
        ...
        metrics: <String>[
          ...
          // Slice A11.1.b â€” session-record gauge consumer (per-instance
...
class ProxyHealthRegistryContext {
  ...
  /// Block 2 (Lock 7 v1) â€” optional accessor into the per-instance
  ...
  /// Slice A11.1.b â€” optional accessor into the per-instance
...
class RegistryProxyHealthCheckStore implements ProxyHealthCheckStore {
  ...
  /// Slice A11.1.b â€” optional accessor for the in-process
...
/// Default dependency probe â€” three small queries, each independently
/// budget-bounded.
///
...
/// HARD-A â€” production-grade dependency probe.
///
...
/// count is not the success criterion â€” `MATCH (n)` against an empty
...
Future<ProxyHealthDependencyProbe> strictProxyHealthDependencyProbe(
  ...
  // instead of prefixing a `SET search_path` â€” same effect, one
  ...
  final results = await Future.wait(<Future<bool>>[
    ...
    // returns zero rows but does not raise â€” still green.
...
/// mapping is missing â€” idempotent, no-op when already configured.
...
/// producer has a registry to compare against. Idempotent â€” uses
...
class ProxyMigrationApplyRegistryWriter {
  ...
  Future<int> recordAppliedMigrations(
    ...
    for (final filename in migrationFilenames) {
      // Defensive: refuse anything but a basename â€” the registry is
