...
class RepositoryMobileOperationalSyncProxyGateway
  ...
  static Map<String, String> _bodyCoversSourcePerServicePeriod(
    Map<String, Object?> body,
  ) {
    const field = 'covers_source_per_service_period';
    final raw = body[field];
    if (raw == null) return const <String, String>{};
    if (raw is! Map) {
      throw const MobileOperationalSyncProxyGatewayException(
        statusCode: 400,
        code: 'invalid_covers_source_per_service_period',
        message: 'covers_source_per_service_period must be an object',
      );
    }
    final out = <String, String>{};
    raw.forEach((key, value) {
      if (key is! String || !_isServicePeriodKey(key.trim())) {
        throw const MobileOperationalSyncProxyGatewayException(
          statusCode: 400,
      ...
      out[key.trim()] = _servicePeriodCoversSourceFromWire(value.trim());
    });
    return out;
  }
  ...
  static String _bodyServicePeriodCoversSource(Map<String, Object?> body) {
    ...
    return _servicePeriodCoversSourceFromWire(value);
  }

  static bool _isServicePeriodKey(String value) {
    return RegExp(r'^[a-z][a-z0-9_]{0,63}$').hasMatch(value);
  }

  static String _servicePeriodCoversSourceFromWire(String value) {
...
class RepositoryPricingTierAdminProxyGateway
  ...
  Future<bool> deleteUsageCap({
    ...
    // When deleting by the logical key the org-unit axes must match the
    // ones the upsert wrote (corp pays for corp scope), so resolve the
    // operator's root org_units row exactly as `upsertUsageCap` does.
    // When a cap_id is supplied the surrogate key alone identifies the
    // row (the repo still scopes the predicate by operator_id), so the
    // org-unit lookup is skipped.
    String orgUnitId = '';
    if (capId == null) {
    ...
    final deleted = await _caps.deleteCap(
      operatorId: operatorId,
      billingOwnerOrgUnitId: orgUnitId,
      scopedOrgUnitId: orgUnitId,
      locationId: locationId,
    ...
    if (deleted) {
      await _audit(
        ...
        payload: <String, Object?>{
          ...
          if (usageClass.isNotEmpty) 'usage_class': usageClass,
          if (staffId != null) 'staff_id': staffId,
          if (workflowId != null) 'workflow_id': workflowId,
        },
      );
  ...
  Future<List<Map<String, Object?>>?> monthToDateSpendSummary({
    ...
    if (existing == null) return null;
    final rows = await _caps.monthToDateSpendByClass(
      operatorId: operatorId,
      adminReason: adminReason,
    );
    await _audit(
