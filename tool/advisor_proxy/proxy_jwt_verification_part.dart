// Forge & Flow advisor proxy — JWT verification classes (part of advisor_proxy.dart).
//
// chore(advisor-proxy): pure size refactor. This part file holds the
// JWT verification interface, concrete verifier implementations, and
// cryptographic helpers mechanically lifted out of `advisor_proxy.dart`
// so the monolith stays under the `kAdvisorProxyMaxLines` bleed-stop
// ceiling enforced by `tool/advisor_proxy_size_lint.dart`. It is a Dart
// `part` of the `advisor_proxy.dart` library: it shares that library's
// imports and private scope verbatim, so the move is behavior
// byte-identical — routing, request/response shapes, status codes,
// error handling, idempotency, RLS, auth, and SQL are all unchanged.
// No symbol was renamed; every helper reference resolves through the
// shared library scope exactly as before.
