...
class _TickOutcome {
  ...
  const _TickOutcome.drainedWithResult(MfaRemovalWorkerResult result)
    : drained = true,
      tickResult = result;
  const _TickOutcome.drainedTimeout() : drained = true, tickResult = null;
...
Future<_TickOutcome> _awaitTickWithDrain(
  ...
  Object? tickError;
  final tickDone = tickFuture.then(
    (value) {
      tickResult = value;
    },
  ...
  await Future.any<void>(<Future<void>>[tickDone, shutdown.future]);
...
class _ShutdownFlag {
  ...
  void requestShutdown(String name) {
  ...
  try {
    subs.add(
      ProcessSignal.sigterm.watch().listen(
        (_) => shutdown.requestShutdown('SIGTERM'),
      ),
    );
  } catch (_) {
  ...
  try {
    subs.add(
      ProcessSignal.sigint.watch().listen(
        (_) => shutdown.requestShutdown('SIGINT'),
      ),
    );
  } catch (_) {
